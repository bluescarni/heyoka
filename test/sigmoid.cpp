...
TEST_CASE("cfunc")
{
    auto tester = [](auto fp_x, unsigned opt_level, bool high_accuracy, bool compact_mode) {
        ...
        for (auto batch_size : {1u, 2u, 4u, 5u}) {
            ...
            cfunc<fp_t> cf({sigmoid(x), sigmoid(expression{fp_t(-.5)}), sigmoid(par[0])}, {x},
            ...
            if (opt_level == 0u && compact_mode) {
                ...
                REQUIRE(std::ranges::any_of(irs, [](const auto &ir) {
                    return boost::contains(ir, "heyoka.llvm_c_eval.sigmoid.");
                }));
            }
            ...
            for (auto i = 0u; i < batch_size; ++i) {
                REQUIRE(outs[i] == approximately(sigmoid(ins[i]), fp_t(100)));
                REQUIRE(outs[i + batch_size] == approximately(sigmoid(static_cast<fp_t>(-.5)), fp_t(100)));
                REQUIRE(outs[i + 2u * batch_size] == approximately(sigmoid(pars[i]), fp_t(100)));
            }
        }
    };
...
TEST_CASE("cfunc_mp")
{
    ...
    for (auto compact_mode : {false, true}) {
        for (auto opt_level : {0u, 1u, 2u, 3u}) {
            cfunc<mppp::real> cf({sigmoid(x), sigmoid(expression{mppp::real{1.5, prec}}), sigmoid(par[0])}, {x},
            ...
            auto i = 0u;
            REQUIRE(outs[i] == sigmoid(ins[i]));
            REQUIRE(outs[i + 1u] == sigmoid(mppp::real{1.5, prec}));
            REQUIRE(outs[i + 2u * 1u] == sigmoid(pars[i]));
        }
    }
}
