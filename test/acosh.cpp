...
TEST_CASE("acosh diff var")
{
    ...
    REQUIRE(diff(acosh(x * x - y), x) == pow(square_wrapper(square_wrapper(x) - y) - 1., -.5) * (2. * x));
...
TEST_CASE("cfunc")
{
    auto tester = [](auto fp_x, unsigned opt_level, bool high_accuracy, bool compact_mode) {
        ...
        for (auto batch_size : {1u, 2u, 4u, 5u}) {
            ...
            add_cfunc<fp_t>(s, "cfunc", {acosh(x), acosh(expression{fp_t(1.5)}), acosh(par[0])}, {x},
            ...
            for (auto i = 0u; i < batch_size; ++i) {
                REQUIRE(outs[i] == approximately(acosh(ins[i]), fp_t(100)));
                REQUIRE(outs[i + batch_size] == approximately(acosh(static_cast<fp_t>(1.5)), fp_t(100)));
                REQUIRE(outs[i + 2u * batch_size] == approximately(acosh(pars[i]), fp_t(100)));
            }
        }
    };
...
TEST_CASE("cfunc_mp")
{
    ...
    for (auto compact_mode : {false, true}) {
        for (auto opt_level : {0u, 1u, 2u, 3u}) {
            ...
            add_cfunc<mppp::real>(s, "cfunc", {acosh(x), acosh(expression{mppp::real{1.5, prec}}), acosh(par[0])}, {x},
            ...
            auto i = 0u;
            REQUIRE(outs[i] == acosh(ins[i]));
            REQUIRE(outs[i + 1u] == acosh(mppp::real{1.5, prec}));
            REQUIRE(outs[i + 2u * 1u] == acosh(pars[i]));
        }
    }
}
...
TEST_CASE("normalise")
{
    ...
    REQUIRE(normalise(acosh(x)) == acosh(x));
...
TEST_CASE("vfabi double")
{
    for (auto fast_math : {false, true}) {
        ...
        add_cfunc<double>(s, "cfunc", {acosh(a), acosh(b)}, {a, b});
        add_cfunc<double>(s, "cfuncs", {acosh(a), acosh(b)}, {a, b}, kw::strided = true);
...
TEST_CASE("vfabi float")
{
    for (auto fast_math : {false, true}) {
        ...
        add_cfunc<float>(s, "cfunc", {acosh(a), acosh(b), acosh(c), acosh(d)}, {a, b, c, d});
        add_cfunc<float>(s, "cfuncs", {acosh(a), acosh(b), acosh(c), acosh(d)}, {a, b, c, d}, kw::strided = true);
