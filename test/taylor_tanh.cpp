...
TEST_CASE("taylor tanh test simplifications")
{
    ...
    taylor_add_jet<double>(s, "jet", {tanh(x + y) * tanh(x + y) + tanh(x + y), x}, 2, 1, false, false);
...
TEST_CASE("taylor tanh")
{
    auto tester = [](auto fp_x, unsigned opt_level, bool high_accuracy, bool compact_mode) {
        ...
        if constexpr (!std::is_same_v<long double, fp_t> || !skip_batch_ld) {
            ...
            compare_batch_scalar<fp_t>({tanh(expression{number{fp_t{2}}}), x + y}, opt_level, high_accuracy,
                                       compact_mode);
        }
