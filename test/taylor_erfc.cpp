...
TEST_CASE("taylor erfc decompose bug 00")
{
    auto x = "x"_var;

    auto ta = taylor_adaptive<double>({prime(x) = erfc(0_dbl) - x}, {0.}, kw::tol = 1.);
}
    ...
    for (auto opt_level : {0u, 1u, 2u, 3u}) {
        for (auto cm : {false, true}) {
            for (auto ha : {false, true}) {
                ...
                taylor_adaptive<double> ta0({prime(x) = erfc(1e-2 * x) + x}, {.5}, kw::high_accuracy = ha,
                                            kw::compact_mode = cm, kw::opt_level = opt_level);
                taylor_adaptive<double> ta1(
                    {prime(x) = s + x, prime(s) = (-2. / sqrt(pi) * exp(-1e-4 * x * x)) * 1e-2 * (s + x)},
                    {.5, erfc(1e-2 * .5)}, kw::high_accuracy = ha, kw::compact_mode = cm, kw::opt_level = opt_level);
                ...
                const auto v0 = erfc(ta0.get_state()[0] * 1e-2);
                const auto v1 = ta1.get_state()[1];

                REQUIRE(abs((v0 - v1) / v0) < 1e-13);
            }
        }
    }
}
...
TEST_CASE("taylor erfc test simplifications")
{
    using std::erfc;
    using std::exp;
    using std::sqrt;
    ...
    auto ta = taylor_adaptive<double>{
        {prime(x) = exp(-(pow(x + y, 2_dbl))) + erfc(x + y), prime(y) = x}, {2., 3.}, kw::opt_level = 0, kw::tol = 1.};
    ...
    REQUIRE(jet[0] == 2.);
    REQUIRE(jet[1] == 3.);
    REQUIRE(jet[2] == approximately(exp(-(jet[0] + jet[1]) * (jet[0] + jet[1])) + erfc(jet[0] + jet[1])));
    REQUIRE(jet[3] == jet[0]);
    REQUIRE(
        jet[4]
        == approximately(.5
...
TEST_CASE("taylor erfc")
{
    auto tester = [](auto fp_x, unsigned opt_level, bool high_accuracy, bool compact_mode) {
        ...
        // Number tests.
        {
            auto ta = taylor_adaptive<fp_t>{{prime(x) = erfc(expression{number{fp_t{2}}}), prime(y) = x + y},
                                            {fp_t(2), fp_t(3)},
                                            kw::tol = 1,
            ...
            REQUIRE(jet[0] == 2);
            REQUIRE(jet[1] == 3);
            REQUIRE(jet[2] == approximately(erfc(fp_t{2})));
            REQUIRE(jet[3] == approximately(jet[0] + jet[1]));
        }

        {
            auto ta = taylor_adaptive<fp_t>{{prime(x) = erfc(par[0]), prime(y) = x + y},
                                            {fp_t(2), fp_t(3)},
                                            kw::tol = 1,
                                            kw::high_accuracy = high_accuracy,
                                            kw::compact_mode = compact_mode,
        ...
        if constexpr (!std::is_same_v<long double, fp_t> || !skip_batch_ld) {
            auto ta = taylor_adaptive_batch<fp_t>{{prime(x) = erfc(expression{number{fp_t{2}}}), prime(y) = x + y},
            ...
            REQUIRE(jet[4] == approximately(erfc(fp_t{2})));
            REQUIRE(jet[5] == approximately(erfc(fp_t{2})));
        ...
        if constexpr (!std::is_same_v<long double, fp_t> || !skip_batch_ld) {
            auto ta = taylor_adaptive_batch<fp_t>{{prime(x) = erfc(par[1]), prime(y) = x + y},
                                                  {fp_t{2}, fp_t{-4}, fp_t{3}, fp_t{5}},
                                                  2,
                                                  kw::tol = 1,
                                                  kw::high_accuracy = high_accuracy,
            ...
            REQUIRE(jet[4] == approximately(erfc(fp_t{3})));
            REQUIRE(jet[5] == approximately(erfc(fp_t{3})));
        ...
        if constexpr (!std::is_same_v<long double, fp_t> || !skip_batch_ld) {
            ...
            REQUIRE(jet[6] == approximately(erfc(fp_t{2})));
            REQUIRE(jet[7] == approximately(erfc(fp_t{2})));
            REQUIRE(jet[8] == approximately(erfc(fp_t{2})));
        ...
        if constexpr (!std::is_same_v<long double, fp_t> || !skip_batch_ld) {
            auto ta = taylor_adaptive_batch<fp_t>{{prime(x) = erfc(par[0]), prime(y) = x + y},
                                                  {fp_t{2}, fp_t{-4}, fp_t{-1}, fp_t{3}, fp_t{5}, fp_t{-2}},
                                                  3,
                                                  kw::tol = .1,
                                                  kw::high_accuracy = high_accuracy,
        ...
        if constexpr (!std::is_same_v<long double, fp_t> || !skip_batch_ld) {
            ...
            compare_batch_scalar<fp_t>({prime(x) = erfc(expression{number{fp_t{2}}}), prime(y) = x + y}, opt_level,
                                       high_accuracy, compact_mode, rng, -10.f, 10.f);
        }

        // Variable tests.
        {
            auto ta = taylor_adaptive<fp_t>{{prime(x) = erfc(y), prime(y) = erfc(x)},
                                            {fp_t(2), fp_t(3)},
                                            kw::tol = 1,
                                            kw::high_accuracy = high_accuracy,
                                            kw::compact_mode = compact_mode,
            ...
            REQUIRE(jet[0] == 2);
            REQUIRE(jet[1] == 3);
            REQUIRE(jet[2] == approximately(erfc(jet[1])));
            REQUIRE(jet[3] == approximately(erfc(jet[0])));
        }

        if constexpr (!std::is_same_v<long double, fp_t> || !skip_batch_ld) {
            auto ta = taylor_adaptive_batch<fp_t>{{prime(x) = erfc(y), prime(y) = erfc(x)},
            ...
            REQUIRE(jet[4] == approximately(erfc(jet[2])));
            REQUIRE(jet[5] == approximately(erfc(jet[3])));

            REQUIRE(jet[6] == approximately(erfc(jet[0])));
            REQUIRE(jet[7] == approximately(erfc(jet[1])));
        }
        ...
        if constexpr (!std::is_same_v<long double, fp_t> || !skip_batch_ld) {
            ...
            REQUIRE(jet[6] == approximately(erfc(jet[3])));
            REQUIRE(jet[7] == approximately(erfc(jet[4])));
            REQUIRE(jet[8] == approximately(erfc(jet[5])));

            REQUIRE(jet[9] == approximately(erfc(jet[0])));
            REQUIRE(jet[10] == approximately(erfc(jet[1])));
            REQUIRE(jet[11] == approximately(erfc(jet[2])));
        ...
        if constexpr (!std::is_same_v<long double, fp_t> || !skip_batch_ld) {
            // Do the batch/scalar comparison.
            compare_batch_scalar<fp_t>({prime(x) = erfc(y), prime(y) = erfc(x)}, opt_level, high_accuracy, compact_mode,
                                       rng, -10.f, 10.f);
        }
    };
