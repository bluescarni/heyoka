...
// currently the case).
TEST_CASE("taylor asin decompose bug 00")
{
    llvm_state s;

    auto x = "x"_var;

    taylor_add_jet<double>(s, "jet", {asin(0_dbl) - x}, 1, 1, false, false);
}

// Test CSE involving hidden dependencies.
TEST_CASE("taylor asin test simplifications")
{
    using std::asin;
    using std::sqrt;
    ...
    taylor_add_jet<double>(s, "jet", {asin(x + y) + sqrt(1. - square(x + y)), x}, 2, 1, false, false);
    ...
    REQUIRE(jet[0] == .2);
    REQUIRE(jet[1] == -.3);
    REQUIRE(jet[2] == approximately(asin(jet[0] + jet[1]) + sqrt(1 - (jet[0] + jet[1]) * (jet[0] + jet[1]))));
    REQUIRE(jet[3] == jet[0]);
    REQUIRE(jet[4]
...
TEST_CASE("taylor asin")
{
    auto tester = [](auto fp_x, unsigned opt_level, bool high_accuracy, bool compact_mode) {
        ...
        // Number tests.
        {
            ...
            taylor_add_jet<fp_t>(s, "jet", {asin(expression{number{fp_t{.5}}}), x + y}, 1, 1, high_accuracy,
                                 compact_mode);
            ...
            REQUIRE(jet[0] == 2);
            REQUIRE(jet[1] == 3);
            REQUIRE(jet[2] == approximately(asin(fp_t{.5})));
...

        {
            ...
            taylor_add_jet<fp_t>(s, "jet", {asin(par[0]), x + y}, 1, 1, high_accuracy, compact_mode);
        ...
        if constexpr (!std::is_same_v<long double, fp_t> || !skip_batch_ld) {
            ...
            taylor_add_jet<fp_t>(s, "jet", {asin(expression{number{fp_t{.5}}}), x + y}, 1, 2, high_accuracy,
                                 compact_mode);
            ...
            REQUIRE(jet[4] == approximately(asin(fp_t{.5})));
            REQUIRE(jet[5] == approximately(asin(fp_t{.5})));
        ...
        if constexpr (!std::is_same_v<long double, fp_t> || !skip_batch_ld) {
            ...
            taylor_add_jet<fp_t>(s, "jet", {asin(par[1]), x + y}, 1, 2, high_accuracy, compact_mode);
...

        {
            ...
            taylor_add_jet<fp_t>(s, "jet", {asin(expression{number{fp_t{.25}}}), x + y}, 2, 1, high_accuracy,
                                 compact_mode);
            ...
            REQUIRE(jet[0] == 2);
            REQUIRE(jet[1] == 3);
            REQUIRE(jet[2] == approximately(asin(fp_t{.25})));
            REQUIRE(jet[3] == approximately(jet[0] + jet[1]));
            REQUIRE(jet[4] == 0);
        ...
        if constexpr (!std::is_same_v<long double, fp_t> || !skip_batch_ld) {
            ...
            taylor_add_jet<fp_t>(s, "jet", {asin(expression{number{fp_t{.25}}}), x + y}, 2, 2, high_accuracy,
                                 compact_mode);
            ...
            REQUIRE(jet[4] == approximately(asin(fp_t{.25})));
            REQUIRE(jet[5] == approximately(asin(fp_t{.25})));
        ...
        if constexpr (!std::is_same_v<long double, fp_t> || !skip_batch_ld) {
            ...
            taylor_add_jet<fp_t>(s, "jet", {asin(expression{number{fp_t{0.75}}}), x + y}, 3, 3, high_accuracy,
                                 compact_mode);
            ...
            REQUIRE(jet[6] == approximately(asin(fp_t{0.75})));
            REQUIRE(jet[7] == approximately(asin(fp_t{0.75})));
            REQUIRE(jet[8] == approximately(asin(fp_t{0.75})));
        ...
        if constexpr (!std::is_same_v<long double, fp_t> || !skip_batch_ld) {
            ...
            taylor_add_jet<fp_t>(s, "jet", {asin(par[0]), x + y}, 3, 3, high_accuracy, compact_mode);
            ...
            REQUIRE(jet[6] == approximately(asin(fp_t{.625})));
            REQUIRE(jet[7] == approximately(asin(fp_t{.625})));
            REQUIRE(jet[8] == approximately(asin(fp_t{.625})));
        ...
        if constexpr (!std::is_same_v<long double, fp_t> || !skip_batch_ld) {
            ...
            compare_batch_scalar<fp_t>({asin(expression{number{fp_t{.625}}}), x + y}, opt_level, high_accuracy,
                                       compact_mode);
        }
        ...
        // Variable tests.
        {
            ...
            taylor_add_jet<fp_t>(s, "jet", {asin(y), asin(x)}, 1, 1, high_accuracy, compact_mode);
        ...
        if constexpr (!std::is_same_v<long double, fp_t> || !skip_batch_ld) {
            ...
            taylor_add_jet<fp_t>(s, "jet", {asin(y), asin(x)}, 1, 2, high_accuracy, compact_mode);
            ...
            REQUIRE(jet[6] == approximately(asin(jet[0])));
            REQUIRE(jet[7] == approximately(asin(jet[1])));
        }

        {
            llvm_state s{kw::opt_level = opt_level};

            taylor_add_jet<fp_t>(s, "jet", {asin(y), asin(x)}, 2, 1, high_accuracy, compact_mode);
        ...
        if constexpr (!std::is_same_v<long double, fp_t> || !skip_batch_ld) {
            ...
            taylor_add_jet<fp_t>(s, "jet", {asin(y), asin(x)}, 2, 2, high_accuracy, compact_mode);
        ...
        if constexpr (!std::is_same_v<long double, fp_t> || !skip_batch_ld) {
            ...
            taylor_add_jet<fp_t>(s, "jet", {asin(y), asin(x)}, 3, 3, high_accuracy, compact_mode);
            ...
            REQUIRE(jet[8] == approximately(asin(jet[5])));

            REQUIRE(jet[9] == approximately(asin(jet[0])));
            REQUIRE(jet[10] == approximately(asin(jet[1])));
            REQUIRE(jet[11] == approximately(asin(jet[2])));
        ...
        if constexpr (!std::is_same_v<long double, fp_t> || !skip_batch_ld) {
            // Do the batch/scalar comparison.
            compare_batch_scalar<fp_t>({asin(y), asin(x)}, opt_level, high_accuracy, compact_mode);
        }
    };
