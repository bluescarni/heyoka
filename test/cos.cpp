...
TEST_CASE("cos diff")
{
    ...
    for (const auto combined : {true, false}) {
        REQUIRE(diff(cos(x * x - y, combined), x) == -sin(x * x - y, combined) * (x + x));
        REQUIRE(diff(cos(x * x - y, combined), y) == -(-sin(x * x - y, combined)));

        REQUIRE(diff(cos(par[0] * par[0] - y, combined), par[0])
                == -sin(par[0] * par[0] - y, combined) * (par[0] + par[0]));
        REQUIRE(diff(cos(x * x - par[1], combined), par[1]) == -(-sin(x * x - par[1], combined)));
    }
}
...
TEST_CASE("cos number simpl")
{
    ...
    for (const auto combined : {true, false}) {
        REQUIRE(cos(x * 0., combined) == 1_dbl);
        REQUIRE(cos(0.123_dbl, combined) == expression{std::cos(0.123)});
        // REQUIRE(cos(-0.123_ldbl) == expression{cos(-0.123l)});

#if defined(HEYOKA_HAVE_REAL128)
        using namespace mppp::literals;

        REQUIRE(cos(-0.123_f128, combined) == expression{cos(0.123_rq)});
#endif
    }
}
    ...
    auto tester = [](auto fp_x, unsigned opt_level, bool high_accuracy, bool compact_mode) {
        ...
        for (const auto combined : {true, false}) {
            for (auto batch_size : {1u, 2u, 4u, 5u}) {
                ...
                cfunc<fp_t> cf({cos(x, combined), cos(expression{fp_t(-.5)}, combined), cos(par[0], combined)}, {x},
                ...
                if (opt_level == 0u && compact_mode) {
                    ...
                    REQUIRE(std::ranges::any_of(irs, [combined](const auto &ir) {
                        return boost::contains(ir, combined ? "heyoka.llvm_c_eval.combined_cos."
                                                            : "heyoka.llvm_c_eval.cos.");
                    }));
                }
    ...
    for (const auto combined : {true, false}) {
        for (auto compact_mode : {false, true}) {
            for (auto opt_level : {0u, 1u, 2u, 3u}) {
                cfunc<mppp::real> cf(
                    {cos(x, combined), cos(expression{mppp::real{-.5, prec}}, combined), cos(par[0], combined)}, {x},
                ...
                auto i = 0u;
                REQUIRE(outs[i] == cos(ins[i]));
                REQUIRE(outs[i + 1u] == cos(mppp::real{-.5, prec}));
                REQUIRE(outs[i + 2u * 1u] == cos(pars[i]));
            }
        }
    }
}
...
TEST_CASE("vfabi double")
{
    for (const auto combined : {true, false}) {
        for (auto fast_math : {false, true}) {
            ...
            add_cfunc<double>(s, "cfunc", {cos(a, combined), cos(b, combined)}, {a, b});
            add_cfunc<double>(s, "cfuncs", {cos(a, combined), cos(b, combined)}, {a, b}, kw::strided = true);
...
TEST_CASE("vfabi float")
{
    for (const auto combined : {true, false}) {
        for (auto fast_math : {false, true}) {
            ...
            add_cfunc<float>(s, "cfunc", {cos(a, combined), cos(b, combined), cos(c, combined), cos(d, combined)},
                             {a, b, c, d});
            add_cfunc<float>(s, "cfuncs", {cos(a, combined), cos(b, combined), cos(c, combined), cos(d, combined)},
