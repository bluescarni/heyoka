...
TEST_CASE("atanh diff")
{
    ...
    REQUIRE(diff(atanh(square_wrapper(x) - y), x) == (2_dbl * x) / (1. - square_wrapper(square_wrapper(x) - y)));
    REQUIRE(diff(atanh(square_wrapper(x) + y), y) == 1. / (1. - square_wrapper(square_wrapper(x) + y)));

    REQUIRE(diff(atanh(square_wrapper(par[0]) - y), par[0])
            == (2_dbl * par[0]) / (1. - square_wrapper(square_wrapper(par[0]) - y)));
    REQUIRE(diff(atanh(square_wrapper(x) + par[1]), par[1]) == 1. / (1. - square_wrapper(square_wrapper(x) + par[1])));
}
...
TEST_CASE("cfunc")
{
    auto tester = [](auto fp_x, unsigned opt_level, bool high_accuracy, bool compact_mode) {
        ...
        for (auto batch_size : {1u, 2u, 4u, 5u}) {
            ...
            add_cfunc<fp_t>(s, "cfunc", {atanh(x), atanh(expression{fp_t(-.5)}), atanh(par[0])}, {x},
...
TEST_CASE("cfunc_mp")
{
    ...
    for (auto compact_mode : {false, true}) {
        for (auto opt_level : {0u, 1u, 2u, 3u}) {
            ...
            add_cfunc<mppp::real>(s, "cfunc", {atanh(x), atanh(expression{mppp::real{.5, prec}}), atanh(par[0])}, {x},
...
TEST_CASE("vfabi double")
{
    for (auto fast_math : {false, true}) {
        ...
        add_cfunc<double>(s, "cfunc", {atanh(a), atanh(b)}, {a, b});
        add_cfunc<double>(s, "cfuncs", {atanh(a), atanh(b)}, {a, b}, kw::strided = true);
...
TEST_CASE("vfabi float")
{
    for (auto fast_math : {false, true}) {
        ...
        add_cfunc<float>(s, "cfunc", {atanh(a), atanh(b), atanh(c), atanh(d)}, {a, b, c, d});
        add_cfunc<float>(s, "cfuncs", {atanh(a), atanh(b), atanh(c), atanh(d)}, {a, b, c, d}, kw::strided = true);
