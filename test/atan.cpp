...
TEST_CASE("atan diff")
{
    ...
    REQUIRE(diff(atan(par[0] * par[0] - y), par[0]) == (2. * par[0]) / (1. + square(square(par[0]) - y)));
    REQUIRE(diff(atan(x * x + par[1]), par[1]) == 1. / (1. + square(square(x) + par[1])));
}
...
TEST_CASE("cfunc")
{
    auto tester = [](auto fp_x, unsigned opt_level, bool high_accuracy, bool compact_mode) {
        ...
        for (auto batch_size : {1u, 2u, 4u, 5u}) {
            ...
            add_cfunc<fp_t>(s, "cfunc", {atan(x), atan(expression{fp_t(-.5)}), atan(par[0])},
...
TEST_CASE("cfunc_mp")
{
    ...
    for (auto compact_mode : {false, true}) {
        for (auto opt_level : {0u, 1u, 2u, 3u}) {
            ...
            add_cfunc<mppp::real>(s, "cfunc", {atan(x), atan(expression{mppp::real{1.5, prec}}), atan(par[0])},
