...
TEST_CASE("atan diff")
{
    ...
    REQUIRE(diff(atan(x * x - y), x) == (x + x) / (1. + square_wrapper(square_wrapper(x) - y)));
    REQUIRE(diff(atan(x * x + y), y) == 1. / (1. + square_wrapper(square_wrapper(x) + y)));

    REQUIRE(diff(atan(par[0] * par[0] - y), par[0])
            == (par[0] + par[0]) / (1. + square_wrapper(square_wrapper(par[0]) - y)));
...
TEST_CASE("cfunc")
{
    auto tester = [](auto fp_x, unsigned opt_level, bool high_accuracy, bool compact_mode) {
        ...
        for (auto batch_size : {1u, 2u, 4u, 5u}) {
            ...
            add_cfunc<fp_t>(s, "cfunc", {atan(x), atan(expression{fp_t(-.5)}), atan(par[0])}, {x},
            ...
            for (auto i = 0u; i < batch_size; ++i) {
                REQUIRE(outs[i] == approximately(atan(ins[i]), fp_t(100)));
                REQUIRE(outs[i + batch_size] == approximately(atan(static_cast<fp_t>(-.5)), fp_t(100)));
                REQUIRE(outs[i + 2u * batch_size] == approximately(atan(pars[i]), fp_t(100)));
            }
        }
    };
...
TEST_CASE("cfunc_mp")
{
    ...
    for (auto compact_mode : {false, true}) {
        for (auto opt_level : {0u, 1u, 2u, 3u}) {
            ...
            add_cfunc<mppp::real>(s, "cfunc", {atan(x), atan(expression{mppp::real{1.5, prec}}), atan(par[0])}, {x},
            ...
            auto i = 0u;
            REQUIRE(outs[i] == atan(ins[i]));
            REQUIRE(outs[i + 1u] == atan(mppp::real{1.5, prec}));
            REQUIRE(outs[i + 2u * 1u] == atan(pars[i]));
        }
    }
}
...
TEST_CASE("vfabi double")
{
    for (auto fast_math : {false, true}) {
        ...
        add_cfunc<double>(s, "cfunc", {atan(a), atan(b)}, {a, b});
        add_cfunc<double>(s, "cfuncs", {atan(a), atan(b)}, {a, b}, kw::strided = true);
...
TEST_CASE("vfabi float")
{
    for (auto fast_math : {false, true}) {
        ...
        add_cfunc<float>(s, "cfunc", {atan(a), atan(b), atan(c), atan(d)}, {a, b, c, d});
        add_cfunc<float>(s, "cfuncs", {atan(a), atan(b), atan(c), atan(d)}, {a, b, c, d}, kw::strided = true);
