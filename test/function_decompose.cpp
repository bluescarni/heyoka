...
TEST_CASE("basic explicit")
{
    ...
    // Error handling.
    REQUIRE_THROWS_MATCHES(function_decompose({}, {}, false), std::invalid_argument,
                           Message("Cannot decompose a function with no outputs"));

    auto [x, y, z] = make_vars("x", "y", "z");

    REQUIRE_THROWS_MATCHES(function_decompose({x}, {x, x}, false), std::invalid_argument,
                           Message("Error in the decomposition of a function: the variable 'x' "
                                   "appears in the user-provided list of variables twice"));

    REQUIRE_THROWS_MATCHES(function_decompose({x}, {x, par[0]}, false), std::invalid_argument,
    ...
    REQUIRE_THROWS_MATCHES(function_decompose({x + y}, {x}, false), std::invalid_argument,
    ...
    auto dc = function_decompose({0_dbl}, {}, false);
    REQUIRE(dc.size() == 1u);
    REQUIRE(dc[0] == 0_dbl);

    dc = function_decompose({par[1]}, {}, false);
    REQUIRE(dc.size() == 1u);
    REQUIRE(dc[0] == par[1]);

    // Simple tests.
    dc = function_decompose({x}, {x}, false);
    REQUIRE(dc.size() == 2u);
    REQUIRE(dc[0] == x);
    REQUIRE(dc[1] == "u_0"_var);

    dc = function_decompose({x}, {y, x}, false);
    REQUIRE(dc.size() == 3u);
    REQUIRE(dc[0] == y);
    REQUIRE(dc[1] == x);
    REQUIRE(dc[2] == "u_1"_var);

    dc = function_decompose({x}, {x, y}, false);
    REQUIRE(dc.size() == 3u);
    REQUIRE(dc[0] == x);
    REQUIRE(dc[1] == y);
    REQUIRE(dc[2] == "u_0"_var);

    dc = function_decompose({x, y}, {x, y}, false);
    REQUIRE(dc.size() == 4u);
    REQUIRE(dc[0] == x);
    REQUIRE(dc[1] == y);
    REQUIRE(dc[2] == "u_0"_var);
    ...
    dc = function_decompose({x, y}, {y, x}, false);
    REQUIRE(dc.size() == 4u);
    REQUIRE(dc[0] == y);
    REQUIRE(dc[1] == x);
    REQUIRE(dc[2] == "u_1"_var);
