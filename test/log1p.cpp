...
TEST_CASE("log1p diff")
{
    auto [x, y] = make_vars("x", "y");

    REQUIRE(diff(log1p(x * x - y), x) == pow(1_dbl + (x * x - y), -1_dbl) * (x + x));
    REQUIRE(diff(log1p(x * x - y), y) == -1_dbl / (1_dbl + (x * x - y)));

    REQUIRE(diff(log1p(par[0] * par[0] - y), par[0]) == pow(1_dbl + (par[0] * par[0] - y), -1_dbl) * (par[0] + par[0]));
    REQUIRE(diff(log1p(x * x - par[1]), par[1]) == -1_dbl / (1_dbl + (x * x - par[1])));
}
...
TEST_CASE("cfunc")
{
    auto tester = [](auto fp_x, unsigned opt_level, bool high_accuracy, bool compact_mode) {
        ...
        for (auto batch_size : {1u, 2u, 4u, 5u}) {
            ...
            cfunc<fp_t> cf({log1p(x), log1p(expression{fp_t(.5)}), log1p(par[0])}, {x}, kw::batch_size = batch_size,
            ...
            if (opt_level == 0u && compact_mode) {
                ...
                REQUIRE(std::ranges::any_of(
                    irs, [](const auto &ir) { return boost::contains(ir, "heyoka.llvm_c_eval.log1p."); }));
            }
            ...
            for (auto i = 0u; i < batch_size; ++i) {
                REQUIRE(outs[i] == approximately(log1p(ins[i]), fp_t(100)));
                REQUIRE(outs[i + batch_size] == approximately(log1p(static_cast<fp_t>(.5)), fp_t(100)));
                REQUIRE(outs[i + 2u * batch_size] == approximately(log1p(pars[i]), fp_t(100)));
            }
        }
    };
...
TEST_CASE("cfunc_mp")
{
    ...
    for (auto compact_mode : {false, true}) {
        for (auto opt_level : {0u, 1u, 2u, 3u}) {
            cfunc<mppp::real> cf({log1p(x), log1p(expression{mppp::real{1.5, prec}}), log1p(par[0])}, {x},
            ...
            auto i = 0u;
            REQUIRE(outs[i] == log1p(ins[i]));
            REQUIRE(outs[i + 1u] == log1p(mppp::real{1.5, prec}));
            REQUIRE(outs[i + 2u * 1u] == log1p(pars[i]));
        }
    }
}
...
TEST_CASE("vfabi double")
{
    for (auto fast_math : {false, true}) {
        ...
        add_cfunc<double>(s, "cfunc", {log1p(a), log1p(b)}, {a, b});
        add_cfunc<double>(s, "cfuncs", {log1p(a), log1p(b)}, {a, b}, kw::strided = true);
        ...
        REQUIRE(outs[0] == approximately(std::log1p(1.)));
        REQUIRE(outs[1] == approximately(std::log1p(2.)));
        ...
        auto count = 0u;
        for (auto it = boost::make_find_iterator(ir, boost::first_finder("@log1p", boost::is_iequal()));
...
TEST_CASE("vfabi float")
{
    for (auto fast_math : {false, true}) {
        ...
        add_cfunc<float>(s, "cfunc", {log1p(a), log1p(b), log1p(c), log1p(d)}, {a, b, c, d});
        add_cfunc<float>(s, "cfuncs", {log1p(a), log1p(b), log1p(c), log1p(d)}, {a, b, c, d}, kw::strided = true);
        ...
        REQUIRE(outs[0] == approximately(std::log1p(1.f)));
        REQUIRE(outs[1] == approximately(std::log1p(2.f)));
        REQUIRE(outs[2] == approximately(std::log1p(3.f)));
        REQUIRE(outs[3] == approximately(std::log1p(4.f)));
        ...
        auto count = 0u;
        for (auto it = boost::make_find_iterator(ir, boost::first_finder("@log1pf", boost::is_iequal()));
             it != string_find_iterator(); ++it) {
            ++count;
        }
