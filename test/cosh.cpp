...
TEST_CASE("cosh diff")
{
    ...
    REQUIRE(diff(cosh(x * x - y), x) == sinh(square_wrapper(x) - y) * (x + x));
    REQUIRE(diff(cosh(x * x + y), y) == sinh(square_wrapper(x) + y));

    REQUIRE(diff(cosh(par[0] * par[0] - y), par[0]) == sinh(square_wrapper(par[0]) - y) * (par[0] + par[0]));
    REQUIRE(diff(cosh(x * x + par[1]), par[1]) == sinh(square_wrapper(x) + par[1]));
}
    ...
    auto tester = [](auto fp_x, unsigned opt_level, bool high_accuracy, bool compact_mode) {
        ...
        for (auto batch_size : {1u, 2u, 4u, 5u}) {
            ...
            for (auto i = 0u; i < batch_size; ++i) {
                ...
                REQUIRE(outs[i + batch_size] == approximately(cosh(static_cast<fp_t>(-.5)), fp_t(100)));
                REQUIRE(outs[i + 2u * batch_size] == approximately(cosh(pars[i]), fp_t(100)));
            }
        }
    };
...
TEST_CASE("cfunc_mp")
{
    ...
    for (auto compact_mode : {false, true}) {
        for (auto opt_level : {0u, 1u, 2u, 3u}) {
            ...
            REQUIRE(outs[i + 1u] == cosh(mppp::real{1.5, prec}));
            REQUIRE(outs[i + 2u * 1u] == cosh(pars[i]));
        }
    }
}
