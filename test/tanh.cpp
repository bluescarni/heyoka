...
TEST_CASE("cfunc")
{
    auto tester = [](auto fp_x, unsigned opt_level, bool high_accuracy, bool compact_mode) {
        ...
        for (auto batch_size : {1u, 2u, 4u, 5u}) {
            ...
            cfunc<fp_t> cf({tanh(x), tanh(expression{fp_t(-.5)}), tanh(par[0])}, {x},
            ...
            if (opt_level == 0u && compact_mode) {
                ...
                REQUIRE(std::ranges::any_of(irs, [](const auto &ir) {
                    return boost::contains(ir, "heyoka.llvm_c_eval.tanh.");
                }));
            }
...
TEST_CASE("cfunc_mp")
{
    ...
    for (auto compact_mode : {false, true}) {
        for (auto opt_level : {0u, 1u, 2u, 3u}) {
            cfunc<mppp::real> cf({tanh(x), tanh(expression{mppp::real{-.5, prec}}), tanh(par[0])}, {x},
...
TEST_CASE("vfabi double")
{
    for (auto fast_math : {false, true}) {
        ...
        add_cfunc<double>(s, "cfunc", {tanh(a), tanh(b)}, {a, b});
        add_cfunc<double>(s, "cfuncs", {tanh(a), tanh(b)}, {a, b}, kw::strided = true);
...
TEST_CASE("vfabi float")
{
    for (auto fast_math : {false, true}) {
        ...
        add_cfunc<float>(s, "cfunc", {tanh(a), tanh(b), tanh(c), tanh(d)}, {a, b, c, d});
        add_cfunc<float>(s, "cfuncs", {tanh(a), tanh(b), tanh(c), tanh(d)}, {a, b, c, d}, kw::strided = true);
