...
TEST_CASE("asin diff var")
{
    ...
    REQUIRE(diff(asin(square_wrapper(x) - y), x) == pow(1. - square_wrapper(square_wrapper(x) - y), -.5) * (2_dbl * x));
    REQUIRE(diff(asin(square_wrapper(x) + y), y) == pow(1. - square_wrapper(square_wrapper(x) + y), -.5));
}
...
TEST_CASE("asin diff par")
{
    ...
    REQUIRE(diff(asin(square_wrapper(par[0]) - y), par[0])
            == pow(1. - square_wrapper(square_wrapper(par[0]) - y), -.5) * (2_dbl * par[0]));
    REQUIRE(diff(asin(square_wrapper(x) + par[1]), par[1])
...
TEST_CASE("cfunc")
{
    auto tester = [](auto fp_x, unsigned opt_level, bool high_accuracy, bool compact_mode) {
        ...
        for (auto batch_size : {1u, 2u, 4u, 5u}) {
            ...
            cfunc<fp_t> cf({asin(x), asin(expression{fp_t(-.5)}), asin(par[0])}, {x}, kw::batch_size = batch_size,
            ...
            if (opt_level == 0u && compact_mode) {
                ...
                REQUIRE(std::ranges::any_of(
                    irs, [](const auto &ir) { return boost::contains(ir, "heyoka.llvm_c_eval.asin."); }));
            }
            ...
            for (auto i = 0u; i < batch_size; ++i) {
                REQUIRE(outs[i] == approximately(asin(ins[i]), fp_t(100)));
                REQUIRE(outs[i + batch_size] == approximately(asin(static_cast<fp_t>(-.5)), fp_t(100)));
                REQUIRE(outs[i + 2u * batch_size] == approximately(asin(pars[i]), fp_t(100)));
            }
        }
    };
...
TEST_CASE("cfunc_mp")
{
    ...
    for (auto compact_mode : {false, true}) {
        for (auto opt_level : {0u, 1u, 2u, 3u}) {
            cfunc<mppp::real> cf({asin(x), asin(expression{mppp::real{-.5, prec}}), asin(par[0])}, {x},
            ...
            auto i = 0u;
            REQUIRE(outs[i] == asin(ins[i]));
            REQUIRE(outs[i + 1u] == asin(mppp::real{-.5, prec}));
            REQUIRE(outs[i + 2u * 1u] == asin(pars[i]));
        }
    }
}
...
TEST_CASE("vfabi double")
{
    for (auto fast_math : {false, true}) {
        ...
        add_cfunc<double>(s, "cfunc", {asin(a), asin(b)}, {a, b});
        add_cfunc<double>(s, "cfuncs", {asin(a), asin(b)}, {a, b}, kw::strided = true);
...
TEST_CASE("vfabi float")
{
    for (auto fast_math : {false, true}) {
        ...
        add_cfunc<float>(s, "cfunc", {asin(a), asin(b), asin(c), asin(d)}, {a, b, c, d});
        add_cfunc<float>(s, "cfuncs", {asin(a), asin(b), asin(c), asin(d)}, {a, b, c, d}, kw::strided = true);
