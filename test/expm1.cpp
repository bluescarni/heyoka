...
TEST_CASE("expm1")
{
    ...
    stream << expm1(x);
    REQUIRE(stream.str() == "expm1(x)");
}

TEST_CASE("expm1 diff")
{
    auto [x, y] = make_vars("x", "y");

    REQUIRE(diff(expm1(x * x - y), x) == exp(x * x - y) * (x + x));
    REQUIRE(diff(expm1(x * x - y), y) == -exp(x * x - y));

    REQUIRE(diff(expm1(par[0] * par[0] - y), par[0]) == exp(par[0] * par[0] - y) * (par[0] + par[0]));
    REQUIRE(diff(expm1(x * x - par[1]), par[1]) == -exp(x * x - par[1]));
}

TEST_CASE("expm1 s11n")
{
    std::stringstream ss;
...
TEST_CASE("cfunc")
{
    auto tester = [](auto fp_x, unsigned opt_level, bool high_accuracy, bool compact_mode) {
        ...
        for (auto batch_size : {1u, 2u, 4u, 5u}) {
            ...
            cfunc<fp_t> cf({expm1(x), expm1(expression{fp_t(-.5)}), expm1(par[0])}, {x}, kw::batch_size = batch_size,
            ...
            if (opt_level == 0u && compact_mode) {
                ...
                REQUIRE(std::ranges::any_of(
                    irs, [](const auto &ir) { return boost::contains(ir, "heyoka.llvm_c_eval.expm1."); }));
            }
            ...
            for (auto i = 0u; i < batch_size; ++i) {
                REQUIRE(outs[i] == approximately(expm1(ins[i]), fp_t(100)));
                REQUIRE(outs[i + batch_size] == approximately(expm1(static_cast<fp_t>(-.5)), fp_t(100)));
                REQUIRE(outs[i + 2u * batch_size] == approximately(expm1(pars[i]), fp_t(100)));
            }
        }
    };
...
TEST_CASE("cfunc_mp")
{
    ...
    for (auto compact_mode : {false, true}) {
        for (auto opt_level : {0u, 1u, 2u, 3u}) {
            cfunc<mppp::real> cf({expm1(x), expm1(expression{mppp::real{1.5, prec}}), expm1(par[0])}, {x},
            ...
            auto i = 0u;
            REQUIRE(outs[i] == expm1(ins[i]));
            REQUIRE(outs[i + 1u] == expm1(mppp::real{1.5, prec}));
            REQUIRE(outs[i + 2u * 1u] == expm1(pars[i]));
        }
    }
}
...
TEST_CASE("vfabi double")
{
    for (auto fast_math : {false, true}) {
        ...
        add_cfunc<double>(s, "cfunc", {expm1(a), expm1(b)}, {a, b});
        add_cfunc<double>(s, "cfuncs", {expm1(a), expm1(b)}, {a, b}, kw::strided = true);
        ...
        REQUIRE(outs[0] == approximately(std::expm1(1.)));
        REQUIRE(outs[1] == approximately(std::expm1(2.)));
        ...
        auto count = 0u;
        for (auto it = boost::make_find_iterator(ir, boost::first_finder("@expm1", boost::is_iequal()));
...
TEST_CASE("vfabi float")
{
    for (auto fast_math : {false, true}) {
        ...
        add_cfunc<float>(s, "cfunc", {expm1(a), expm1(b), expm1(c), expm1(d)}, {a, b, c, d});
        add_cfunc<float>(s, "cfuncs", {expm1(a), expm1(b), expm1(c), expm1(d)}, {a, b, c, d}, kw::strided = true);
        ...
        REQUIRE(outs[0] == approximately(std::expm1(1.f)));
        REQUIRE(outs[1] == approximately(std::expm1(2.f)));
        REQUIRE(outs[2] == approximately(std::expm1(3.f)));
        REQUIRE(outs[3] == approximately(std::expm1(4.f)));
        ...
        auto count = 0u;
        for (auto it = boost::make_find_iterator(ir, boost::first_finder("@expm1f", boost::is_iequal()));
             it != string_find_iterator(); ++it) {
            ++count;
        }
