...
// The expected line length in a USNO EOP data file.
constexpr auto eop_data_usno_expected_line_length = 185u;

// Helper to parse the MJD from a line in a USNO EOP data file.
double parse_eop_data_usno_mjd(auto cur_line)
{
    static_assert(std::ranges::contiguous_range<decltype(cur_line)>);
    assert(std::ranges::size(cur_line) >= eop_data_usno_expected_line_length);
    ...
    if (mjd_parse_res.ec != std::errc{} || mjd_parse_res.ptr != mjd_end) [[unlikely]] {
        throw std::invalid_argument(
            fmt::format("Error parsing a USNO EOP data file: the string '{}' could not be parsed as a valid MJD",
...
// Helper to parse the UT1-UTC difference from a line in a USNO EOP data file.
std::optional<double> parse_eop_data_usno_delta_ut1_utc(auto cur_line)
{
    static_assert(std::ranges::contiguous_range<decltype(cur_line)>);
    assert(std::ranges::size(cur_line) >= eop_data_usno_expected_line_length);
    ...
    if (begin != end) {
        ...
        if (parse_res.ec != std::errc{} || parse_res.ptr != end) [[unlikely]] {
            throw std::invalid_argument(
                fmt::format("Error parsing a USNO EOP data file: the bulletin B string for the UT1-UTC "
    ...
    if (parse_res.ec != std::errc{} || parse_res.ptr != end) [[unlikely]] {
        throw std::invalid_argument(
            fmt::format("Error parsing a USNO EOP data file: the bulletin A string for the UT1-UTC "
                        "difference '{}' could not be parsed as a floating-point value",
                        bullA_sv));
    }
...
eop_data_table parse_eop_data_usno(const std::string &str)
{
    // Parse line by line, splitting on newlines.
    eop_data_table retval;
    for (const auto cur_line : str | std::views::split('\n')) {
        // NOTE: USNO EOP data files may have a newline at the end. When we encounter it, just break out.
        if (std::ranges::empty(cur_line)) {
            break;
        }

        // Check the line length.
        if (std::ranges::size(cur_line) < detail::eop_data_usno_expected_line_length) [[unlikely]] {
            throw std::invalid_argument(fmt::format(
                "Invalid line detected in a USNO EOP data file: the expected number of "
                "characters in the line is at least {}, but a line with {} character(s) was detected instead",
                detail::eop_data_usno_expected_line_length, std::ranges::size(cur_line)));
        }

        // Parse the mjd.
        const auto mjd = detail::parse_eop_data_usno_mjd(cur_line);

        // Parse the UT1-UTC difference.
        const auto delta_ut1_utc = detail::parse_eop_data_usno_delta_ut1_utc(cur_line);
        if (!delta_ut1_utc) {
...
const std::set<std::string> eop_data_usno_filenames
...
eop_data eop_data::fetch_latest_usno(const std::string &filename)
{
    // Check the provided filename.
    if (!detail::eop_data_usno_filenames.contains(filename)) [[unlikely]] {
        throw std::invalid_argument(
            fmt::format("Invalid filename '{}' specified for a USNO EOP data file: the valid names are {}", filename,
                        detail::eop_data_usno_filenames));
    }
