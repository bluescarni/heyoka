...
{
    return llvm_eval_helper([&s](const std::vector<llvm::Value *> &args, bool) { return llvm_acos(s, args[0]); }, *this,
...
[[nodiscard]] llvm::Function *acos_llvm_c_eval(llvm_state &s, llvm::Type *fp_t, const func_base &fb,
                                               std::uint32_t batch_size, bool high_accuracy)
{
    return llvm_c_eval_func_helper(
        "acos", [&s](const std::vector<llvm::Value *> &args, bool) { return llvm_acos(s, args[0]); }, fb, s, fp_t,
        batch_size, high_accuracy);
}

} // namespace

llvm::Function *acos_impl::llvm_c_eval_func(llvm_state &s, llvm::Type *fp_t, std::uint32_t batch_size,
                                            bool high_accuracy) const
{
    return acos_llvm_c_eval(s, fp_t, *this, batch_size, high_accuracy);
}
...
taylor_dc_t::size_type acos_impl::taylor_decompose(taylor_dc_t &u_vars_defs) &&
{
    ...
    // - otherwise 1 - arg * arg is not any more an elementary
    //   subexpression,
    // - if 1 - arg * arg appears elsewhere in the expression, then
    //   it is automatically transformed into a sub() by the decomposition
    //   preprocessing.
    u_vars_defs.emplace_back(sub(1_dbl, expression{fmt::format("u_{}", u_vars_defs.size() - 1u)}),
...
// All the other cases.
template <typename U, std::enable_if_t<!is_num_param_v<U>, int> = 0>
llvm::Value *taylor_diff_acos_impl(llvm_state &, llvm::Type *, const acos_impl &, const std::vector<std::uint32_t> &,
...
llvm::Value *taylor_diff_acos(llvm_state &s, llvm::Type *fp_t, const acos_impl &f,
    ...
    if (deps.size() != 1u) {
        throw std::invalid_argument(
            fmt::format("A hidden dependency vector of size 1 is expected in order to compute the Taylor "
                        "derivative of the inverse cosine, but a vector of size {} was passed instead",
                        deps.size()));
    }

    return std::visit(
        [&](const auto &v) {
            return taylor_diff_acos_impl(s, fp_t, f, deps, v, arr, par_ptr, n_uvars, order, idx, batch_size);
        },
        f.args()[0].value());
}
...
llvm::Value *acos_impl::taylor_diff(llvm_state &s, llvm::Type *fp_t, const std::vector<std::uint32_t> &deps,
                                    const std::vector<llvm::Value *> &arr, llvm::Value *par_ptr, llvm::Value *,
                                    std::uint32_t n_uvars, std::uint32_t order, std::uint32_t idx,
                                    std::uint32_t batch_size, bool) const
{
    return taylor_diff_acos(s, fp_t, *this, deps, arr, par_ptr, n_uvars, order, idx, batch_size);
}
                                             ...
                                             std::uint32_t n_uvars, std::uint32_t batch_size)
{
    ...
    const auto na_pair = taylor_c_diff_func_name_args(context, fp_t, "acos", n_uvars, batch_size, {var}, 1);
    ...
    if (f == nullptr) {
        ...
        llvm_if_then_else(
            ...
            [&]() {
                ...
                builder.CreateStore(
                    llvm_acos(s, taylor_c_load_diff(s, val_t, diff_ptr, n_uvars, builder.getInt32(0), b_idx)), retval);
            },
            [&]() {
