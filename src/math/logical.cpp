...
llvm::Value *logical_or_eval_impl(llvm_state &s, const std::vector<llvm::Value *> &args)
{
    assert(!args.empty());
...
{
    return llvm_eval_helper(
        [&s](const std::vector<llvm::Value *> &args, bool) { return logical_or_eval_impl(s, args); }, *this, s, fp_t,
        eval_arr, par_ptr, stride, batch_size, high_accuracy);
}
...
{
    return llvm_c_eval_func_helper(
        "logical_or", [&s](const std::vector<llvm::Value *> &args, bool) { return logical_or_eval_impl(s, args); },
        *this, s, fp_t, batch_size, high_accuracy);
}
