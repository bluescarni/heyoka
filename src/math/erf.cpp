...
llvm::Value *erf_impl::llvm_evaluate(llvm_state &s, const std::vector<llvm::Value *> &args, llvm::Type *, llvm::Value *,
                                     bool)
{
    assert(args.size() == 1u);
...
taylor_dc_t::size_type erf_common_taylor_decompose(auto &self, taylor_dc_t &u_vars_defs)
{
    ...
    // Append arg * arg.
    u_vars_defs.emplace_back(pow(self.args()[0], 2_dbl), std::vector<std::uint32_t>{});
    ...
    // Append the erf/erfc decomposition.
    u_vars_defs.emplace_back(func{std::move(self)}, std::vector<std::uint32_t>{});
...
{
    return erf_common_taylor_decompose(*this, u_vars_defs);
}
                                         ...
                                         const auto &evaluator, const number &cfac)
{
    ...
    if (order == 0u) {
        return evaluator(s, taylor_fetch_diff(arr, b_idx, 0, n_uvars));
    }
    ...
    // Generate the factor n * cfac.
    const auto fac = number_like(s, fp_t, static_cast<double>(order)) * cfac;
                                    ...
                                    std::uint32_t batch_size, const auto &evaluator, const number &cfac)
{
    ...
    return std::visit(
        [&](const auto &v) {
            return taylor_diff_erf_common_impl(s, fp_t, f, deps, v, arr, par_ptr, n_uvars, order, idx, batch_size,
                                               evaluator, cfac);
        },
        f.args()[0].value());
}
...
{
    return taylor_diff_erf_common(s, fp_t, *this, deps, arr, par_ptr, n_uvars, order, idx, batch_size, &llvm_erf,
                                  sqrt_pi_2_like(s, fp_t));
}
...
// Derivative of erf(number)/erfc(number).
template <typename U, std::enable_if_t<is_num_param_v<U>, int> = 0>
llvm::Function *taylor_c_diff_func_erf_common_impl(llvm_state &s, llvm::Type *fp_t, const auto &self, const U &num,
                                                   std::uint32_t n_uvars, std::uint32_t batch_size,
                                                   const auto &evaluator, const number &)
{
    return taylor_c_diff_func_numpar(
        s, fp_t, n_uvars, batch_size, self.get_name(), 1,
        [&s, &evaluator](const auto &args) {
            // LCOV_EXCL_START
            assert(args.size() == 1u);
            assert(args[0] != nullptr);
            // LCOV_EXCL_STOP

            return evaluator(s, args[0]);
        },
        num);
}

// Derivative of erf(variable)/erfc(variable).
llvm::Function *taylor_c_diff_func_erf_common_impl(llvm_state &s, llvm::Type *fp_t, const auto &self,
                                                   ...
                                                   const auto &evaluator, const number &cfac)
{
    ...
    const auto na_pair = taylor_c_diff_func_name_args(context, fp_t, self.get_name(), n_uvars, batch_size, {var}, 1);
    ...
    if (f == nullptr) {
        ...
        llvm_if_then_else(
            ...
            [&] {
                ...
                builder.CreateStore(
                    evaluator(s, taylor_c_load_diff(s, val_t, diff_ptr, n_uvars, builder.getInt32(0), b_idx)), retval);
            },
            [&] {
                ...
                // Generate the factor n * cfac.
                auto *t1_llvm = vector_splat(builder, llvm_codegen(s, fp_t, cfac), batch_size);
                                              ...
                                              std::uint32_t batch_size, const auto &evaluator, const number &cfac)
{
    ...
    return std::visit(
        [&](const auto &v) {
            return taylor_c_diff_func_erf_common_impl(s, fp_t, fn, v, n_uvars, batch_size, evaluator, cfac);
        },
        fn.args()[0].value());
}
...
llvm::Function *erf_impl::taylor_c_diff_func(llvm_state &s, llvm::Type *fp_t, std::uint32_t n_uvars,
                                             std::uint32_t batch_size, bool) const
{
    return taylor_c_diff_func_erf_common(s, fp_t, *this, n_uvars, batch_size, &llvm_erf, sqrt_pi_2_like(s, fp_t));
}
...
erfc_impl::erfc_impl(expression e) : func_base("erfc", std::vector{std::move(e)}) {}

erfc_impl::erfc_impl() : erfc_impl(0_dbl) {}

llvm::Value *erfc_impl::llvm_evaluate(llvm_state &s, const std::vector<llvm::Value *> &args, llvm::Type *,
                                      llvm::Value *, bool)
{
    assert(args.size() == 1u);
    return llvm_erfc(s, args[0]);
}

taylor_dc_t::size_type erfc_impl::taylor_decompose(taylor_dc_t &u_vars_defs) &&
{
    return erf_common_taylor_decompose(*this, u_vars_defs);
}

llvm::Value *erfc_impl::taylor_diff(llvm_state &s, llvm::Type *fp_t, const std::vector<std::uint32_t> &deps,
                                    const std::vector<llvm::Value *> &arr, llvm::Value *par_ptr, llvm::Value *,
                                    std::uint32_t n_uvars, std::uint32_t order, std::uint32_t idx,
                                    std::uint32_t batch_size, bool) const
{
    return taylor_diff_erf_common(s, fp_t, *this, deps, arr, par_ptr, n_uvars, order, idx, batch_size, &llvm_erfc,
                                  -sqrt_pi_2_like(s, fp_t));
}

llvm::Function *erfc_impl::taylor_c_diff_func(llvm_state &s, llvm::Type *fp_t, std::uint32_t n_uvars,
                                              std::uint32_t batch_size, bool) const
{
    return taylor_c_diff_func_erf_common(s, fp_t, *this, n_uvars, batch_size, &llvm_erfc, -sqrt_pi_2_like(s, fp_t));
}

std::vector<expression> erfc_impl::gradient() const
{
    assert(args().size() == 1u);
    return {-2_dbl / sqrt(pi) * exp(-pow(args()[0], 2_dbl))};
}
    ...
    if (const auto *num_ptr = std::get_if<number>(&e.value())) {
        return std::visit(
            [](const auto &x) {
                ...
                return expression{erfc(x)};
            },
            num_ptr->value());
    } else {
        return expression{func{detail::erfc_impl(std::move(e))}};
    }
}
...
// NOLINTNEXTLINE(cert-err58-cpp,bugprone-throwing-static-initialization)
HEYOKA_S11N_FUNC_EXPORT_IMPLEMENT(heyoka::detail::erfc_impl)
