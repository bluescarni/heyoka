...
{
    if (order == 0u) {
        return llvm_sin(s, taylor_codegen_numparam<T>(s, num, par_ptr, batch_size));
    } else {
    ...
    if (f == nullptr) {
        ...
        llvm_if_then_else(
            ...
            [&]() {
                ...
                builder.CreateStore(llvm_sin(s, taylor_c_load_diff(s, diff_ptr, n_uvars, builder.getInt32(0), var_idx)),
                                    retval);
            },
            [&]() {
                // Init the accumlator.
