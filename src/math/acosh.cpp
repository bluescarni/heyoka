...
expression acosh_impl::diff(const std::string &s) const
{
    assert(args().size() == 1u);
...
taylor_dc_t::size_type acosh_impl::taylor_decompose(taylor_dc_t &u_vars_defs) &&
{
    ...
    // Append arg * arg - 1.
    u_vars_defs.push_back(taylor_dc_item_t{expression{"u_{}"_format(u_vars_defs.size() - 1u)} - 1_dbl, {}, {}, {}});
                                              ...
                                              std::uint32_t batch_size)
{
    return taylor_c_diff_func_unary_num_det<T>(
        ...
        "the inverse hyperbolic cosine", 1);
}

// Derivative of acosh(variable).
template <typename T>
llvm::Function *taylor_c_diff_func_acosh_impl(llvm_state &s, const acosh_impl &fn, const variable &,
                                              std::uint32_t n_uvars, std::uint32_t batch_size)
{
    ...
    // - idx of the uvar whose definition is sqrt(var * var - 1).
