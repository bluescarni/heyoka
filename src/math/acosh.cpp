...
llvm::Value *acosh_impl::codegen_dbl(llvm_state &s, const std::vector<llvm::Value *> &args) const
{
    ...
    if (auto vec_t = llvm::dyn_cast<llvm_vector_type>(args[0]->getType())) {
        if (const auto sfn = sleef_function_name(s.context(), "acosh", vec_t->getElementType(),
    ...
    return call_extern_vec(s, args[0], "acosh");
}

llvm::Value *acosh_impl::codegen_ldbl(llvm_state &s, const std::vector<llvm::Value *> &args) const
{
    assert(args.size() == 1u);
    assert(args[0] != nullptr);
...
#if defined(_MSC_VER)
                           // NOTE: it seems like the MSVC stdlib does not have an acoshl function,
...
llvm::Value *acosh_impl::codegen_f128(llvm_state &s, const std::vector<llvm::Value *> &args) const
{
    assert(args.size() == 1u);
    assert(args[0] != nullptr);
...
llvm::Value *taylor_diff_acosh_impl(llvm_state &s, const acosh_impl &f, const std::vector<std::uint32_t> &,
...
template <typename T>
llvm::Value *taylor_diff_acosh_impl(llvm_state &s, const acosh_impl &f, const std::vector<std::uint32_t> &deps,
...
// Derivative of acosh(number).
template <typename T, typename U, std::enable_if_t<is_num_param_v<U>, int> = 0>
llvm::Function *taylor_c_diff_func_acosh_impl(llvm_state &s, const acosh_impl &fn, const U &num, std::uint32_t n_uvars,
                                              std::uint32_t, std::uint32_t batch_size)
{
    return taylor_c_diff_func_unary_num_det<T>(s, fn, num, n_uvars, batch_size, "acosh", 1);
}

// Derivative of acosh(variable).
template <typename T>
llvm::Function *taylor_c_diff_func_acosh_impl(llvm_state &s, const acosh_impl &fn, const variable &var,
                                         ...
                                         std::uint32_t order, std::uint32_t batch_size)
{
    ...
    return std::visit(
        [&](const auto &v) { return taylor_c_diff_func_acosh_impl<T>(s, fn, v, n_uvars, order, batch_size); },
        fn.args()[0].value());
}

} // namespace

llvm::Function *acosh_impl::taylor_c_diff_func_dbl(llvm_state &s, std::uint32_t n_uvars, std::uint32_t order,
                                                   std::uint32_t batch_size, bool) const
{
    return taylor_c_diff_func_acosh<double>(s, *this, n_uvars, order, batch_size);
}

llvm::Function *acosh_impl::taylor_c_diff_func_ldbl(llvm_state &s, std::uint32_t n_uvars, std::uint32_t order,
                                                    std::uint32_t batch_size, bool) const
{
    return taylor_c_diff_func_acosh<long double>(s, *this, n_uvars, order, batch_size);
}

#if defined(HEYOKA_HAVE_REAL128)

llvm::Function *acosh_impl::taylor_c_diff_func_f128(llvm_state &s, std::uint32_t n_uvars, std::uint32_t order,
                                                    std::uint32_t batch_size, bool) const
{
    return taylor_c_diff_func_acosh<mppp::real128>(s, *this, n_uvars, order, batch_size);
}
