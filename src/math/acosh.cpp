...
{
    return llvm_eval_helper([&s](const std::vector<llvm::Value *> &args, bool) { return llvm_acosh(s, args[0]); },
...
[[nodiscard]] llvm::Function *acosh_llvm_c_eval(llvm_state &s, llvm::Type *fp_t, const func_base &fb,
                                                std::uint32_t batch_size, bool high_accuracy)
{
    return llvm_c_eval_func_helper(
        "acosh", [&s](const std::vector<llvm::Value *> &args, bool) { return llvm_acosh(s, args[0]); }, fb, s, fp_t,
        batch_size, high_accuracy);
}

} // namespace

llvm::Function *acosh_impl::llvm_c_eval_func(llvm_state &s, llvm::Type *fp_t, std::uint32_t batch_size,
                                             bool high_accuracy) const
{
    return acosh_llvm_c_eval(s, fp_t, *this, batch_size, high_accuracy);
}
...
llvm::Value *taylor_diff_acosh_impl(llvm_state &s, llvm::Type *fp_t, const acosh_impl &,
...
// All the other cases.
template <typename U, std::enable_if_t<!is_num_param_v<U>, int> = 0>
llvm::Value *taylor_diff_acosh_impl(llvm_state &, llvm::Type *, const acosh_impl &, const std::vector<std::uint32_t> &,
...
llvm::Value *taylor_diff_acosh(llvm_state &s, llvm::Type *fp_t, const acosh_impl &f,
                               ...
                               std::uint32_t batch_size)
{
    ...
    return std::visit(
        [&](const auto &v) {
            return taylor_diff_acosh_impl(s, fp_t, f, deps, v, arr, par_ptr, n_uvars, order, idx, batch_size);
        },
        f.args()[0].value());
}
...
llvm::Value *acosh_impl::taylor_diff(llvm_state &s, llvm::Type *fp_t, const std::vector<std::uint32_t> &deps,
                                     const std::vector<llvm::Value *> &arr, llvm::Value *par_ptr, llvm::Value *,
                                     std::uint32_t n_uvars, std::uint32_t order, std::uint32_t idx,
                                     std::uint32_t batch_size, bool) const
{
    return taylor_diff_acosh(s, fp_t, *this, deps, arr, par_ptr, n_uvars, order, idx, batch_size);
}
...
// Derivative of acosh(number).
template <typename U, std::enable_if_t<is_num_param_v<U>, int> = 0>
llvm::Function *taylor_c_diff_func_acosh_impl(llvm_state &s, llvm::Type *fp_t, const acosh_impl &, const U &num,
                                              std::uint32_t n_uvars, std::uint32_t batch_size)
{
    return taylor_c_diff_func_numpar(
        s, fp_t, n_uvars, batch_size, "acosh", 1,
        [&s](const auto &args) {
            // LCOV_EXCL_START
            assert(args.size() == 1u);
...
llvm::Function *taylor_c_diff_func_acosh_impl(llvm_state &s, llvm::Type *fp_t, const acosh_impl &, const variable &var,
                                              std::uint32_t n_uvars, std::uint32_t batch_size)
{
    ...
    const auto na_pair = taylor_c_diff_func_name_args(context, fp_t, "acosh", n_uvars, batch_size, {var}, 1);
...
// All the other cases.
template <typename U, std::enable_if_t<!is_num_param_v<U>, int> = 0>
llvm::Function *taylor_c_diff_func_acosh_impl(llvm_state &, llvm::Type *, const acosh_impl &, const U &, std::uint32_t,
                                              std::uint32_t)
{
    throw std::invalid_argument("An invalid argument type was encountered while trying to build the Taylor derivative "
...
llvm::Function *taylor_c_diff_func_acosh(llvm_state &s, llvm::Type *fp_t, const acosh_impl &fn, std::uint32_t n_uvars,
                                         std::uint32_t batch_size)
{
    assert(fn.args().size() == 1u);

    return std::visit([&](const auto &v) { return taylor_c_diff_func_acosh_impl(s, fp_t, fn, v, n_uvars, batch_size); },
                      fn.args()[0].value());
}

} // namespace

llvm::Function *acosh_impl::taylor_c_diff_func(llvm_state &s, llvm::Type *fp_t, std::uint32_t n_uvars,
                                               std::uint32_t batch_size, bool) const
{
    return taylor_c_diff_func_acosh(s, fp_t, *this, n_uvars, batch_size);
}
