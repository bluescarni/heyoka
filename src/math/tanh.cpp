...
// Derivative of tanh(number).
template <typename T, typename U, std::enable_if_t<is_num_param_v<U>, int> = 0>
llvm::Function *taylor_c_diff_func_tanh_impl(llvm_state &s, const tanh_impl &fn, const U &num, std::uint32_t n_uvars,
                                             std::uint32_t, std::uint32_t batch_size)
{
    return taylor_c_diff_func_unary_num_det<T>(s, fn, num, n_uvars, batch_size, "tanh", 1);
}

// Derivative of tanh(variable).
template <typename T>
llvm::Function *taylor_c_diff_func_tanh_impl(llvm_state &s, const tanh_impl &fn, const variable &var,
    ...
    if (f == nullptr) {
        ...
        llvm_if_then_else(
            ...
            [&]() {
                ...
                builder.CreateStore(builder.CreateFSub(taylor_c_load_diff(s, diff_ptr, n_uvars, ord, b_idx, order),
...
template <typename T>
llvm::Function *taylor_c_diff_func_tanh(llvm_state &s, const tanh_impl &fn, std::uint32_t n_uvars, std::uint32_t order,
                                        std::uint32_t batch_size)
{
    assert(fn.args().size() == 1u);

    return std::visit(
        [&](const auto &v) { return taylor_c_diff_func_tanh_impl<T>(s, fn, v, n_uvars, order, batch_size); },
        fn.args()[0].value());
}

} // namespace

llvm::Function *tanh_impl::taylor_c_diff_func_dbl(llvm_state &s, std::uint32_t n_uvars, std::uint32_t order,
                                                  std::uint32_t batch_size, bool) const
{
    return taylor_c_diff_func_tanh<double>(s, *this, n_uvars, order, batch_size);
}

llvm::Function *tanh_impl::taylor_c_diff_func_ldbl(llvm_state &s, std::uint32_t n_uvars, std::uint32_t order,
                                                   std::uint32_t batch_size, bool) const
{
    return taylor_c_diff_func_tanh<long double>(s, *this, n_uvars, order, batch_size);
}

#if defined(HEYOKA_HAVE_REAL128)

llvm::Function *tanh_impl::taylor_c_diff_func_f128(llvm_state &s, std::uint32_t n_uvars, std::uint32_t order,
                                                   std::uint32_t batch_size, bool) const
{
    return taylor_c_diff_func_tanh<mppp::real128>(s, *this, n_uvars, order, batch_size);
}
