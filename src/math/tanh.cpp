...
expression tanh_impl::diff(std::unordered_map<const void *, expression> &func_map, const std::string &s) const
{
    assert(args().size() == 1u);

    return (1_dbl - square(tanh(args()[0]))) * detail::diff(func_map, args()[0], s);
}
