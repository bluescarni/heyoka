...
llvm::Value *asin_impl::llvm_eval_dbl(llvm_state &s, const std::vector<llvm::Value *> &eval_arr, llvm::Value *par_ptr,
                                      llvm::Value *stride, std::uint32_t batch_size, bool high_accuracy) const
{
    return llvm_eval_helper<double>(
        [&s](const std::vector<llvm::Value *> &args, bool) { return llvm_asin(s, args[0]); }, *this, s, eval_arr,
        par_ptr, stride, batch_size, high_accuracy);
}

llvm::Value *asin_impl::llvm_eval_ldbl(llvm_state &s, const std::vector<llvm::Value *> &eval_arr, llvm::Value *par_ptr,
                                       llvm::Value *stride, std::uint32_t batch_size, bool high_accuracy) const
{
...
llvm::Value *asin_impl::llvm_eval_f128(llvm_state &s, const std::vector<llvm::Value *> &eval_arr, llvm::Value *par_ptr,
                                       llvm::Value *stride, std::uint32_t batch_size, bool high_accuracy) const
{
...
template <typename T>
[[nodiscard]] llvm::Function *asin_llvm_c_eval(llvm_state &s, const func_base &fb, std::uint32_t batch_size,
                                               bool high_accuracy)
{
    return llvm_c_eval_func_helper<T>(
        "asin", [&s](const std::vector<llvm::Value *> &args, bool) { return llvm_asin(s, args[0]); }, fb, s, batch_size,
        high_accuracy);
}

} // namespace

llvm::Function *asin_impl::llvm_c_eval_func_dbl(llvm_state &s, std::uint32_t batch_size, bool high_accuracy) const
{
    return asin_llvm_c_eval<double>(s, *this, batch_size, high_accuracy);
}

llvm::Function *asin_impl::llvm_c_eval_func_ldbl(llvm_state &s, std::uint32_t batch_size, bool high_accuracy) const
{
    return asin_llvm_c_eval<long double>(s, *this, batch_size, high_accuracy);
}

#if defined(HEYOKA_HAVE_REAL128)

llvm::Function *asin_impl::llvm_c_eval_func_f128(llvm_state &s, std::uint32_t batch_size, bool high_accuracy) const
{
    return asin_llvm_c_eval<mppp::real128>(s, *this, batch_size, high_accuracy);
}
...
llvm::Value *taylor_diff_asin_impl(llvm_state &s, const asin_impl &, const std::vector<std::uint32_t> &, const U &num,
                                   const std::vector<llvm::Value *> &, llvm::Value *par_ptr, std::uint32_t,
                                   std::uint32_t order, std::uint32_t, std::uint32_t batch_size)
{
    if (order == 0u) {
        return llvm_asin(s, taylor_codegen_numparam<T>(s, num, par_ptr, batch_size));
    } else {
...
template <typename T>
llvm::Value *taylor_diff_asin_impl(llvm_state &s, const asin_impl &, const std::vector<std::uint32_t> &deps,
...
// All the other cases.
template <typename T, typename U, std::enable_if_t<!is_num_param_v<U>, int> = 0>
llvm::Value *taylor_diff_asin_impl(llvm_state &, const asin_impl &, const std::vector<std::uint32_t> &, const U &,
...
template <typename T>
llvm::Value *taylor_diff_asin(llvm_state &s, const asin_impl &f, const std::vector<std::uint32_t> &deps,
                              ...
                              std::uint32_t order, std::uint32_t idx, std::uint32_t batch_size)
{
    ...
    if (deps.size() != 1u) {
        throw std::invalid_argument(
            ...
            "derivative of the inverse sine, but a vector of size {} was passed instead"_format(deps.size()));
    }

    return std::visit(
        [&](const auto &v) {
            return taylor_diff_asin_impl<T>(s, f, deps, v, arr, par_ptr, n_uvars, order, idx, batch_size);
        },
        f.args()[0].value());
}
...
template <typename T>
llvm::Function *taylor_c_diff_func_asin_impl(llvm_state &s, const asin_impl &, const variable &var,
                                             std::uint32_t n_uvars, std::uint32_t batch_size)
{
    ...
    const auto na_pair = taylor_c_diff_func_name_args<T>(context, "asin", n_uvars, batch_size, {var}, 1);
    ...
    if (f == nullptr) {
        ...
        llvm_if_then_else(
            ...
            [&]() {
                ...
                builder.CreateStore(llvm_asin(s, taylor_c_load_diff(s, diff_ptr, n_uvars, builder.getInt32(0), b_idx)),
                                    retval);
            },
            [&]() {
...
// All the other cases.
template <typename T, typename U, std::enable_if_t<!is_num_param_v<U>, int> = 0>
llvm::Function *taylor_c_diff_func_asin_impl(llvm_state &, const asin_impl &, const U &, std::uint32_t, std::uint32_t)
{
...
template <typename T>
llvm::Function *taylor_c_diff_func_asin(llvm_state &s, const asin_impl &fn, std::uint32_t n_uvars,
                                        std::uint32_t batch_size)
{
    assert(fn.args().size() == 1u);

    return std::visit([&](const auto &v) { return taylor_c_diff_func_asin_impl<T>(s, fn, v, n_uvars, batch_size); },
                      fn.args()[0].value());
}

} // namespace

llvm::Function *asin_impl::taylor_c_diff_func_dbl(llvm_state &s, std::uint32_t n_uvars, std::uint32_t batch_size,
                                                  bool) const
{
    return taylor_c_diff_func_asin<double>(s, *this, n_uvars, batch_size);
}

llvm::Function *asin_impl::taylor_c_diff_func_ldbl(llvm_state &s, std::uint32_t n_uvars, std::uint32_t batch_size,
                                                   bool) const
{
    return taylor_c_diff_func_asin<long double>(s, *this, n_uvars, batch_size);
}

#if defined(HEYOKA_HAVE_REAL128)

llvm::Function *asin_impl::taylor_c_diff_func_f128(llvm_state &s, std::uint32_t n_uvars, std::uint32_t batch_size,
                                                   bool) const
{
    return taylor_c_diff_func_asin<mppp::real128>(s, *this, n_uvars, batch_size);
}
