...
expression asin_impl::diff(const std::string &s) const
{
    assert(args().size() == 1u);
...
llvm::Value *asin_impl::codegen_dbl(llvm_state &s, const std::vector<llvm::Value *> &args) const
{
    ...
    if (auto vec_t = llvm::dyn_cast<llvm::VectorType>(args[0]->getType())) {
        if (const auto sfn = sleef_function_name(s.context(), "asin", vec_t->getElementType(),
...
llvm::Value *asin_impl::codegen_f128(llvm_state &s, const std::vector<llvm::Value *> &args) const
{
    assert(args.size() == 1u);
    assert(args[0] != nullptr);
...
long double asin_impl::eval_ldbl(const std::unordered_map<std::string, long double> &map, const std::vector<long double> &pars) const
{
    assert(args().size() == 1u);
...
#if defined(HEYOKA_HAVE_REAL128)
mppp::real128 asin_impl::eval_f128(const std::unordered_map<std::string, mppp::real128> &map, const std::vector<mppp::real128> &pars) const
{
    assert(args().size() == 1u);
...
asin_impl::taylor_decompose(std::vector<std::pair<expression, std::vector<std::uint32_t>>> &u_vars_defs) &&
{
    assert(args().size() == 1u);
    ...
    u_vars_defs.emplace_back(asin(arg), std::vector<std::uint32_t>{});
...
llvm::Value *taylor_diff_asin_impl(llvm_state &s, const asin_impl &f, const std::vector<std::uint32_t> &, const U &num,
...
template <typename T>
llvm::Value *taylor_diff_asin_impl(llvm_state &s, const asin_impl &f, const std::vector<std::uint32_t> &deps,
...
// Derivative of asin(number).
template <typename T, typename U, std::enable_if_t<is_num_param_v<U>, int> = 0>
llvm::Function *taylor_c_diff_func_asin_impl(llvm_state &s, const asin_impl &fn, const U &num, std::uint32_t,
                                             std::uint32_t batch_size)
{
    using namespace fmt::literals;
    ...
    return taylor_c_diff_func_unary_num_det<T>(
        s, fn, num, batch_size,
        "heyoka_taylor_diff_asin_{}_{}"_format(taylor_c_diff_numparam_mangle(num),
...
template <typename T>
llvm::Function *taylor_c_diff_func_asin_impl(llvm_state &s, const asin_impl &fn, const variable &,
                                             std::uint32_t n_uvars, std::uint32_t batch_size)
{
    ...
    const auto fname
        = "heyoka_taylor_diff_asin_var_" + taylor_mangle_suffix(val_t) + "_n_uvars_" + li_to_string(n_uvars);
...
llvm::Function *asin_impl::taylor_c_diff_func_dbl(llvm_state &s, std::uint32_t n_uvars, std::uint32_t batch_size) const
{
    return taylor_c_diff_func_asin<double>(s, *this, n_uvars, batch_size);
}

llvm::Function *asin_impl::taylor_c_diff_func_ldbl(llvm_state &s, std::uint32_t n_uvars, std::uint32_t batch_size) const
{
...
llvm::Function *asin_impl::taylor_c_diff_func_f128(llvm_state &s, std::uint32_t n_uvars, std::uint32_t batch_size) const
{
