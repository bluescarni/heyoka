...
template <typename T>
llvm::Function *taylor_c_diff_func_sinh(llvm_state &s, const sinh_impl &fn, std::uint32_t n_uvars, std::uint32_t order,
                                        std::uint32_t batch_size)
{
    assert(fn.args().size() == 1u);

    return std::visit(
        [&](const auto &v) { return taylor_c_diff_func_sinh_impl<T>(s, fn, v, n_uvars, order, batch_size); },
        fn.args()[0].value());
}

} // namespace

llvm::Function *sinh_impl::taylor_c_diff_func_dbl(llvm_state &s, std::uint32_t n_uvars, std::uint32_t order,
                                                  std::uint32_t batch_size, bool) const
{
    return taylor_c_diff_func_sinh<double>(s, *this, n_uvars, order, batch_size);
}

llvm::Function *sinh_impl::taylor_c_diff_func_ldbl(llvm_state &s, std::uint32_t n_uvars, std::uint32_t order,
                                                   std::uint32_t batch_size, bool) const
{
    return taylor_c_diff_func_sinh<long double>(s, *this, n_uvars, order, batch_size);
}

#if defined(HEYOKA_HAVE_REAL128)

llvm::Function *sinh_impl::taylor_c_diff_func_f128(llvm_state &s, std::uint32_t n_uvars, std::uint32_t order,
                                                   std::uint32_t batch_size, bool) const
{
    return taylor_c_diff_func_sinh<mppp::real128>(s, *this, n_uvars, order, batch_size);
}
