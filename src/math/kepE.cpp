...
llvm::Value *kepE_impl::llvm_evaluate(llvm_state &s, const std::vector<llvm::Value *> &args, llvm::Type *val_t,
                                      llvm::Value *, bool)
{
    assert(args.size() == 2u);
    ...
    auto *kepE_func = llvm_add_inv_kep_E(s, val_t->getScalarType(), batch_size);
