...
// Derivative of atan(number).
template <typename T, typename U, std::enable_if_t<is_num_param_v<U>, int> = 0>
llvm::Function *taylor_c_diff_func_atan_impl(llvm_state &s, const atan_impl &fn, const U &num, std::uint32_t n_uvars,
                                             std::uint32_t, std::uint32_t batch_size)
{
    return taylor_c_diff_func_unary_num_det<T>(s, fn, num, n_uvars, batch_size, "atan", 1);
}

// Derivative of atan(variable).
template <typename T>
llvm::Function *taylor_c_diff_func_atan_impl(llvm_state &s, const atan_impl &fn, const variable &var,
                                             std::uint32_t n_uvars, std::uint32_t order, std::uint32_t batch_size)
{
    ...
    const auto na_pair = taylor_c_diff_func_name_args<T>(context, "atan", n_uvars, batch_size, {var}, 1);
        ...
        llvm_if_then_else(
            ...
            [&]() {
                ...
                auto n_c0_p1 = builder.CreateFMul(
                    ord_fp, builder.CreateFAdd(
                                taylor_c_load_diff(s, diff_ptr, n_uvars, builder.getInt32(0), c_idx, order), one_fp));
...
template <typename T>
llvm::Function *taylor_c_diff_func_atan(llvm_state &s, const atan_impl &fn, std::uint32_t n_uvars, std::uint32_t order,
                                        std::uint32_t batch_size)
{
    assert(fn.args().size() == 1u);

    return std::visit(
        [&](const auto &v) { return taylor_c_diff_func_atan_impl<T>(s, fn, v, n_uvars, order, batch_size); },
        fn.args()[0].value());
}

} // namespace

llvm::Function *atan_impl::taylor_c_diff_func_dbl(llvm_state &s, std::uint32_t n_uvars, std::uint32_t order,
                                                  std::uint32_t batch_size, bool) const
{
    return taylor_c_diff_func_atan<double>(s, *this, n_uvars, order, batch_size);
}

llvm::Function *atan_impl::taylor_c_diff_func_ldbl(llvm_state &s, std::uint32_t n_uvars, std::uint32_t order,
                                                   std::uint32_t batch_size, bool) const
{
    return taylor_c_diff_func_atan<long double>(s, *this, n_uvars, order, batch_size);
}

#if defined(HEYOKA_HAVE_REAL128)

llvm::Function *atan_impl::taylor_c_diff_func_f128(llvm_state &s, std::uint32_t n_uvars, std::uint32_t order,
                                                   std::uint32_t batch_size, bool) const
{
    return taylor_c_diff_func_atan<mppp::real128>(s, *this, n_uvars, order, batch_size);
}
