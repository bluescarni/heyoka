...
llvm::Value *asinh_impl::codegen_f128(llvm_state &s, const std::vector<llvm::Value *> &args) const
{
    ...
    return call_extern_vec(s, args[0], "heyoka_asinh128");
}
...
long double asinh_impl::eval_ldbl(const std::unordered_map<std::string, long double> &map, const std::vector<long double> &pars) const
{
    assert(args().size() == 1u);
...
#if defined(HEYOKA_HAVE_REAL128)
mppp::real128 asinh_impl::eval_f128(const std::unordered_map<std::string, mppp::real128> &map, const std::vector<mppp::real128> &pars) const
{
    assert(args().size() == 1u);
...
asinh_impl::taylor_decompose(std::vector<std::pair<expression, std::vector<std::uint32_t>>> &u_vars_defs) &&
{
    ...
    u_vars_defs.emplace_back(asinh(arg), std::vector<std::uint32_t>{});
