        ...
        llvm_if_then_else(
            ...
            [&]() {
                ...
                builder.CreateStore(codegen_from_values<T>(
                                        s, fn,
                                        {taylor_c_load_diff(s, diff_ptr, n_uvars, builder.getInt32(0), var_idx, order),
            ...
            [&]() {
                ...
                llvm_loop_u32(s, builder.getInt32(0), ord, [&](llvm::Value *j) {
                    auto b_nj = taylor_c_load_diff(s, diff_ptr, n_uvars, builder.CreateSub(ord, j), var_idx, order);
                ...
                builder.CreateStore(
                    builder.CreateFDiv(
                        ...
                        builder.CreateFMul(
                            ord_v, taylor_c_load_diff(s, diff_ptr, n_uvars, builder.getInt32(0), var_idx, order))),
                    retval);
            });
...
template <typename T>
llvm::Function *taylor_c_diff_func_pow(llvm_state &s, const pow_impl &fn, std::uint32_t n_uvars, std::uint32_t order,
                                       std::uint32_t batch_size)
{
    ...
    return std::visit(
        [&](const auto &v1, const auto &v2) {
            return taylor_c_diff_func_pow_impl<T>(s, fn, v1, v2, n_uvars, order, batch_size);
        },
...
llvm::Function *pow_impl::taylor_c_diff_func_dbl(llvm_state &s, std::uint32_t n_uvars, std::uint32_t order,
                                                 std::uint32_t batch_size, bool) const
{
    return taylor_c_diff_func_pow<double>(s, *this, n_uvars, order, batch_size);
}

llvm::Function *pow_impl::taylor_c_diff_func_ldbl(llvm_state &s, std::uint32_t n_uvars, std::uint32_t order,
                                                  std::uint32_t batch_size, bool) const
{
    return taylor_c_diff_func_pow<long double>(s, *this, n_uvars, order, batch_size);
}

#if defined(HEYOKA_HAVE_REAL128)

llvm::Function *pow_impl::taylor_c_diff_func_f128(llvm_state &s, std::uint32_t n_uvars, std::uint32_t order,
                                                  std::uint32_t batch_size, bool) const
{
    return taylor_c_diff_func_pow<mppp::real128>(s, *this, n_uvars, order, batch_size);
}
