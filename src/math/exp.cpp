...
template <typename T>
llvm::Function *taylor_c_diff_func_exp(llvm_state &s, const exp_impl &fn, std::uint32_t n_uvars, std::uint32_t order,
                                       std::uint32_t batch_size)
{
    assert(fn.args().size() == 1u);

    return std::visit(
        [&](const auto &v) { return taylor_c_diff_func_exp_impl<T>(s, fn, v, n_uvars, order, batch_size); },
        fn.args()[0].value());
}

} // namespace

llvm::Function *exp_impl::taylor_c_diff_func_dbl(llvm_state &s, std::uint32_t n_uvars, std::uint32_t order,
                                                 std::uint32_t batch_size, bool) const
{
    return taylor_c_diff_func_exp<double>(s, *this, n_uvars, order, batch_size);
}

llvm::Function *exp_impl::taylor_c_diff_func_ldbl(llvm_state &s, std::uint32_t n_uvars, std::uint32_t order,
                                                  std::uint32_t batch_size, bool) const
{
    return taylor_c_diff_func_exp<long double>(s, *this, n_uvars, order, batch_size);
}

#if defined(HEYOKA_HAVE_REAL128)

llvm::Function *exp_impl::taylor_c_diff_func_f128(llvm_state &s, std::uint32_t n_uvars, std::uint32_t order,
                                                  std::uint32_t batch_size, bool) const
{
    return taylor_c_diff_func_exp<mppp::real128>(s, *this, n_uvars, order, batch_size);
}
