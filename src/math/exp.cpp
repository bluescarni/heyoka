...
llvm::Value *exp_impl::llvm_evaluate(llvm_state &s, const std::vector<llvm::Value *> &args, llvm::Type *, llvm::Value *,
                                     bool)
{
    assert(args.size() == 1u);
...
expm1_impl::expm1_impl(expression e) : func_base("expm1", std::vector{std::move(e)}) {}

expm1_impl::expm1_impl() : expm1_impl(0_dbl) {}

taylor_dc_t::size_type expm1_impl::taylor_decompose(taylor_dc_t &u_vars_defs) &&
{
    assert(args().size() == 1u);

    // Append exp(arg).
    u_vars_defs.emplace_back(exp(args()[0]), std::vector<std::uint32_t>{});

    // Append the expm1 decomposition.
    u_vars_defs.emplace_back(func{std::move(*this)}, std::vector<std::uint32_t>{});

    // Add the hidden dep.
    (u_vars_defs.end() - 1)->second.push_back(boost::numeric_cast<std::uint32_t>(u_vars_defs.size() - 2u));

    // Compute the return value (pointing to the decomposed expm1).
    return u_vars_defs.size() - 1u;
}

std::vector<expression> expm1_impl::gradient() const
{
    assert(args().size() == 1u);
    return {exp(args()[0])};
}

llvm::Value *expm1_impl::llvm_evaluate(llvm_state &s, const std::vector<llvm::Value *> &args, llvm::Type *,
                                       llvm::Value *, bool)
{
    assert(args.size() == 1u);
    return llvm_expm1(s, args[0]);
}

namespace
{

// Derivative of expm1(number).
template <typename U, std::enable_if_t<is_num_param_v<U>, int> = 0>
llvm::Value *taylor_diff_expm1_impl(llvm_state &s, llvm::Type *fp_t, const expm1_impl &,
                                    const std::vector<std::uint32_t> &, const U &num,
                                    const std::vector<llvm::Value *> &, llvm::Value *par_ptr, std::uint32_t,
                                    std::uint32_t order, std::uint32_t, std::uint32_t batch_size)
{
    if (order == 0u) {
        return llvm_expm1(s, taylor_codegen_numparam(s, fp_t, num, par_ptr, batch_size));
    } else {
        return vector_splat(s.builder(), llvm_codegen(s, fp_t, number{0.}), batch_size);
    }
}

// Derivative of expm1(variable).
llvm::Value *taylor_diff_expm1_impl(llvm_state &s, llvm::Type *fp_t, const expm1_impl &,
                                    ...
                                    std::uint32_t order, std::uint32_t, std::uint32_t batch_size)
{
    ...
    if (order == 0u) {
        return llvm_expm1(s, taylor_fetch_diff(arr, b_idx, 0, n_uvars));
    }

    // NOTE: iteration in the [1, order] range.
    std::vector<llvm::Value *> sum;
    for (std::uint32_t j = 1; j <= order; ++j) {
        // NOTE: the only hidden dependency contains the index of the u variable whose definition is exp(var).
...
// All the other cases.
template <typename U, std::enable_if_t<!is_num_param_v<U>, int> = 0>
llvm::Value *taylor_diff_expm1_impl(llvm_state &, llvm::Type *, const expm1_impl &, const std::vector<std::uint32_t> &,
                                    const U &, const std::vector<llvm::Value *> &, llvm::Value *, std::uint32_t,
                                    std::uint32_t, std::uint32_t, std::uint32_t)
{
    throw std::invalid_argument(
        "An invalid argument type was encountered while trying to build the Taylor derivative of expm1");
}

llvm::Value *taylor_diff_expm1(llvm_state &s, llvm::Type *fp_t, const expm1_impl &f,
    ...
    if (deps.size() != 1u) {
        throw std::invalid_argument(
            fmt::format("A hidden dependency vector of size 1 is expected in order to compute the Taylor "
                        "derivative of expm1, but a vector of size {} was passed instead",
                        deps.size()));
    }

    return std::visit(
        [&](const auto &v) {
            return taylor_diff_expm1_impl(s, fp_t, f, deps, v, arr, par_ptr, n_uvars, order, idx, batch_size);
        },
        f.args()[0].value());
}
...
llvm::Value *expm1_impl::taylor_diff(llvm_state &s, llvm::Type *fp_t, const std::vector<std::uint32_t> &deps,
                                     const std::vector<llvm::Value *> &arr, llvm::Value *par_ptr, llvm::Value *,
                                     std::uint32_t n_uvars, std::uint32_t order, std::uint32_t idx,
                                     std::uint32_t batch_size, bool) const
{
    return taylor_diff_expm1(s, fp_t, *this, deps, arr, par_ptr, n_uvars, order, idx, batch_size);
}
...
// Derivative of expm1(number).
template <typename U, std::enable_if_t<is_num_param_v<U>, int> = 0>
llvm::Function *taylor_c_diff_func_expm1_impl(llvm_state &s, llvm::Type *fp_t, const expm1_impl &, const U &num,
                                              std::uint32_t n_uvars, std::uint32_t batch_size)
{
    return taylor_c_diff_func_numpar(
        s, fp_t, n_uvars, batch_size, "expm1", 1,
        [&s](const auto &args) {
            // LCOV_EXCL_START
            assert(args.size() == 1u);
            assert(args[0] != nullptr);
...
// Derivative of expm1(variable).
llvm::Function *taylor_c_diff_func_expm1_impl(llvm_state &s, llvm::Type *fp_t, const expm1_impl &, const variable &var,
                                              std::uint32_t n_uvars, std::uint32_t batch_size)
{
    auto &module = s.module();
    auto &builder = s.builder();
    ...
    // Fetch the function name and arguments.
    const auto na_pair = taylor_c_diff_func_name_args(context, fp_t, "expm1", n_uvars, batch_size, {var}, 1);
    const auto &fname = na_pair.first;
    const auto &fargs = na_pair.second;
    ...
    if (f == nullptr) {
        ...
        auto *diff_ptr = f->args().begin() + 2;
        auto *b_idx = f->args().begin() + 5;
        // NOTE: the only hidden dependency contains the index of the u variable whose definition is exp(b(t)).
        ...
        llvm_if_then_else(
            ...
            [&] {
                // For order 0, invoke the function on the order 0 of b_idx.
                builder.CreateStore(
                    llvm_expm1(s, taylor_c_load_diff(s, val_t, diff_ptr, n_uvars, builder.getInt32(0), b_idx)), retval);
            },
            [&] {
                // Create a vector version of ord.
                auto *ord_fp = vector_splat(builder, llvm_ui_to_fp(s, ord, fp_t), batch_size);
                ...
                // Run the loop.
                llvm_loop_u32(s, builder.getInt32(1), builder.CreateAdd(ord, builder.getInt32(1)), [&](llvm::Value *j) {
                    auto *cnj = taylor_c_load_diff(s, val_t, diff_ptr, n_uvars, builder.CreateSub(ord, j), c_idx);
                    auto *bj = taylor_c_load_diff(s, val_t, diff_ptr, n_uvars, j, b_idx);
...
// All the other cases.
template <typename U, std::enable_if_t<!is_num_param_v<U>, int> = 0>
llvm::Function *taylor_c_diff_func_expm1_impl(llvm_state &, llvm::Type *, const expm1_impl &, const U &, std::uint32_t,
                                              std::uint32_t)
{
    throw std::invalid_argument("An invalid argument type was encountered while trying to build the Taylor derivative "
                                "of expm1 in compact mode");
}

llvm::Function *taylor_c_diff_func_expm1(llvm_state &s, llvm::Type *fp_t, const expm1_impl &fn, std::uint32_t n_uvars,
                                         std::uint32_t batch_size)
{
    assert(fn.args().size() == 1u);

    return std::visit([&](const auto &v) { return taylor_c_diff_func_expm1_impl(s, fp_t, fn, v, n_uvars, batch_size); },
                      fn.args()[0].value());
}

} // namespace

llvm::Function *expm1_impl::taylor_c_diff_func(llvm_state &s, llvm::Type *fp_t, std::uint32_t n_uvars,
                                               std::uint32_t batch_size, bool) const
{
    return taylor_c_diff_func_expm1(s, fp_t, *this, n_uvars, batch_size);
}

} // namespace detail

expression expm1(expression e)
{
    if (const auto *num_ptr = std::get_if<number>(&e.value())) {
        return std::visit(
            [](const auto &x) {
                ...
                return expression{expm1(x)};
            },
            num_ptr->value());
    } else {
        return expression{func{detail::expm1_impl(std::move(e))}};
    }
}
...
// NOLINTNEXTLINE(cert-err58-cpp,bugprone-throwing-static-initialization)
HEYOKA_S11N_FUNC_EXPORT_IMPLEMENT(heyoka::detail::expm1_impl)
