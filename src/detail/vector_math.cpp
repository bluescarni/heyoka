...
sleef_suffixes sleef_get_suffixes(const std::string_view base_name)
{
    ...
    static const boost::unordered_flat_map<std::string_view, sleef_suffixes> sleef_suffixes_map = {
        ...
        {"expm1", {.hp = "u10", .lp = "u10"}}, {"log1p", {.hp = "u10", .lp = "u10"}},
        {"pow", {.hp = "u10", .lp = "u10"}},
...
#ifdef HEYOKA_SLEEF_VSX
constexpr bool sleef_vsx = true;
#else
constexpr bool sleef_vsx = false;
#endif
                               ...
                               const std::string_view sleef_tp, const std::uint32_t idx)
{
    ...
    if (features.avx512f && sleef_avx512) {
        ...
        retval[scalar_name] = {make_sleef_vfinfo(base_simd_width, "advsimd")};
    } else if (features.vsx && sleef_vsx) {
        retval[scalar_name] = {make_sleef_vfinfo(base_simd_width, "vsx")};
    }
}
...
auto make_vf_map()
{
    ...
    add_vfinfo_sleef(retval, "log1pf", "log1p", "f");
    ...
    add_vfinfo_sleef(retval, "log1p", "log1p", "d");
