...
// NOTE: implement a workaround for the serialisation of tuples whose first element is a taylor outcome. We need this
// because Boost.Serialization treats all enums as ints, which is not ok for taylor_outcome (whose underyling type will
// not be an int on most platforms). Because it is not possible to override Boost's enum implementation, we override the
// serialisation of tuples with outcomes as first elements, which is all we need in the serialisation of the batch
// integrator. The implementation below will be preferred over the generic tuple
// s11n because it is more specialised.
//
// NOTE: this workaround is not necessary for the other enums in heyoka because those all have ints as underlying type.
...
void save(Archive &ar, const std::tuple<heyoka::taylor_outcome, Args...> &tup, unsigned)
{
    ...
    // NOTE: this is a right fold, which, in conjunction with the builtin comma operator, ensures that the serialisation
    // of the tuple elements proceeds in the correct order and with the correct sequencing.
...
// Common logic for initialising the compact-mode tape in scalar and batch integrators.
template <typename IData>
void init_cm_tape(IData &idata)
{
    assert(idata.m_cm_tape.empty());

    const auto [sz, al] = idata.m_tape_sa;

    if (idata.m_compact_mode) {
        assert(sz != 0u); // LCOV_EXCL_LINE
        assert(al != 0u); // LCOV_EXCL_LINE

        idata.m_cm_tape = detail::aligned_vector<std::byte>(detail::aligned_allocator<std::byte>(al));
        idata.m_cm_tape.resize(boost::numeric_cast<decltype(idata.m_cm_tape.size())>(sz));
    } else {
        assert(sz == 0u); // LCOV_EXCL_LINE
        assert(al == 0u); // LCOV_EXCL_LINE
    }
...
taylor_adaptive<T>::i_data::i_data(const i_data &other)
    : m_state(other.m_state), m_time(other.m_time),
      ...
      m_vsys_data(other.m_vsys_data ? std::make_unique<detail::vsys_data<T>>(*other.m_vsys_data) : nullptr),
