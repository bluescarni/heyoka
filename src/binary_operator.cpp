...
template <typename T>
llvm::Value *taylor_init_bo_impl(llvm_state &s, const binary_operator &bo, llvm::Value *diff_arr,
                                 std::uint32_t batch_size, bool high_accuracy)
{
    // Do the Taylor init for lhs and rhs.
    auto l = taylor_init<T>(s, bo.lhs(), diff_arr, batch_size, high_accuracy);
    auto r = taylor_init<T>(s, bo.rhs(), diff_arr, batch_size, high_accuracy);
...
llvm::Value *taylor_init_dbl(llvm_state &s, const binary_operator &bo, llvm::Value *diff_arr, std::uint32_t batch_size,
                             bool high_accuracy)
{
    return detail::taylor_init_bo_impl<double>(s, bo, diff_arr, batch_size, high_accuracy);
}

llvm::Value *taylor_init_ldbl(llvm_state &s, const binary_operator &bo, llvm::Value *diff_arr, std::uint32_t batch_size,
                              bool high_accuracy)
{
    return detail::taylor_init_bo_impl<long double>(s, bo, diff_arr, batch_size, high_accuracy);
}

#if defined(HEYOKA_HAVE_REAL128)

llvm::Value *taylor_init_f128(llvm_state &s, const binary_operator &bo, llvm::Value *diff_arr, std::uint32_t batch_size,
                              bool high_accuracy)
{
    return detail::taylor_init_bo_impl<mppp::real128>(s, bo, diff_arr, batch_size, high_accuracy);
}
...
// Derivative of number +- number.
template <bool, typename T>
llvm::Value *bo_taylor_diff2_addsub_impl(llvm_state &s, const number &, const number &, llvm::Value *, std::uint32_t,
...
// Derivative of number +- var.
template <bool AddOrSub, typename T>
llvm::Value *bo_taylor_diff2_addsub_impl(llvm_state &s, const number &, const variable &var, llvm::Value *diff_arr,
                                         std::uint32_t n_uvars, llvm::Value *order, std::uint32_t, std::uint32_t, bool)
{
    auto ret = taylor_load_diff(s, diff_arr, n_uvars, order, s.builder().getInt32(uname_to_index(var.name())));
...
// Derivative of var +- number.
template <bool AddOrSub, typename T>
llvm::Value *bo_taylor_diff2_addsub_impl(llvm_state &s, const variable &var, const number &, llvm::Value *diff_arr,
                                         std::uint32_t n_uvars, llvm::Value *order, std::uint32_t, std::uint32_t, bool)
{
    return taylor_load_diff(s, diff_arr, n_uvars, order, s.builder().getInt32(uname_to_index(var.name())));
}

// Derivative of var +- var.
template <bool AddOrSub, typename T>
llvm::Value *bo_taylor_diff2_addsub_impl(llvm_state &s, const variable &var0, const variable &var1,
                                         llvm::Value *diff_arr, std::uint32_t n_uvars, llvm::Value *order,
                                         std::uint32_t, std::uint32_t, bool)
{
    // TODO builder cache?
    auto v0 = taylor_load_diff(s, diff_arr, n_uvars, order, s.builder().getInt32(uname_to_index(var0.name())));
    auto v1 = taylor_load_diff(s, diff_arr, n_uvars, order, s.builder().getInt32(uname_to_index(var1.name())));
...
// All the other cases.
template <bool, typename, typename V1, typename V2>
llvm::Value *bo_taylor_diff2_addsub_impl(llvm_state &, const V1 &, const V2 &, llvm::Value *, std::uint32_t,
                                         llvm::Value *, std::uint32_t, std::uint32_t, bool)
{
    assert(false);
...
template <typename T>
llvm::Value *bo_taylor_diff2_add(llvm_state &s, const binary_operator &bo, llvm::Value *diff_arr, std::uint32_t n_uvars,
                                 llvm::Value *order, std::uint32_t idx, std::uint32_t batch_size, bool high_accuracy)
{
    return std::visit(
        [&](const auto &v1, const auto &v2) {
            return bo_taylor_diff2_addsub_impl<true, T>(s, v1, v2, diff_arr, n_uvars, order, idx, batch_size,
                                                        high_accuracy);
        },
        bo.lhs().value(), bo.rhs().value());
}
...
template <typename T>
llvm::Value *bo_taylor_diff2_sub(llvm_state &s, const binary_operator &bo, llvm::Value *diff_arr, std::uint32_t n_uvars,
                                 llvm::Value *order, std::uint32_t idx, std::uint32_t batch_size, bool high_accuracy)
{
    return std::visit(
        [&](const auto &v1, const auto &v2) {
            return bo_taylor_diff2_addsub_impl<false, T>(s, v1, v2, diff_arr, n_uvars, order, idx, batch_size,
                                                         high_accuracy);
        },
        bo.lhs().value(), bo.rhs().value());
}
...
// Derivative of number * number.
template <typename T>
llvm::Value *bo_taylor_diff2_mul_impl(llvm_state &s, const number &, const number &, llvm::Value *, std::uint32_t,
                                      llvm::Value *, std::uint32_t, std::uint32_t batch_size, bool)
{
    return create_constant_vector(s.builder(), codegen<T>(s, number{0.}), batch_size);
}
...
// Derivative of var * number.
template <typename T>
llvm::Value *bo_taylor_diff2_mul_impl(llvm_state &s, const variable &var, const number &num, llvm::Value *diff_arr,
                                      std::uint32_t n_uvars, llvm::Value *order, std::uint32_t,
                                      std::uint32_t batch_size, bool)
{
    auto ret = taylor_load_diff(s, diff_arr, n_uvars, order, s.builder().getInt32(uname_to_index(var.name())));
    auto mul = create_constant_vector(s.builder(), codegen<T>(s, num), batch_size);

    return s.builder().CreateFMul(ret, mul);
}

// Derivative of number * var.
template <typename T>
llvm::Value *bo_taylor_diff2_mul_impl(llvm_state &s, const number &num, const variable &var, llvm::Value *diff_arr,
                                      std::uint32_t n_uvars, llvm::Value *order, std::uint32_t idx,
                                      std::uint32_t batch_size, bool high_accuracy)
{
    return bo_taylor_diff2_mul_impl<T>(s, var, num, diff_arr, n_uvars, order, idx, batch_size, high_accuracy);
}

// Derivative of var * var.
template <typename T>
llvm::Value *bo_taylor_diff2_mul_impl(llvm_state &s, const variable &var0, const variable &var1, llvm::Value *diff_arr,
                                      std::uint32_t n_uvars, llvm::Value *order, std::uint32_t,
                                      std::uint32_t batch_size, bool)
{
    ...
    if (f == nullptr) {
        ...
        std::vector<llvm::Type *> fargs{llvm::Type::getInt32Ty(context), llvm::Type::getInt32Ty(context),
                                        llvm::Type::getInt32Ty(context), llvm::PointerType::getUnqual(vec_t)};
        ...
        // Run the loop.
        // TODO overflow check.
        llvm_loop_u32(s, f, builder.getInt32(0), builder.CreateAdd(ord, builder.getInt32(1)), [&](llvm::Value *j) {
            auto b_nj = taylor_load_diff(s, diff_ptr, n_uvars, builder.CreateSub(ord, j), idx0);
            auto cj = taylor_load_diff(s, diff_ptr, n_uvars, j, idx1);
...
// All the other cases.
template <typename, typename V1, typename V2>
llvm::Value *bo_taylor_diff2_mul_impl(llvm_state &, const V1 &, const V2 &, llvm::Value *, std::uint32_t, llvm::Value *,
                                      std::uint32_t, std::uint32_t, bool)
{
    assert(false);
...
template <typename T>
llvm::Value *bo_taylor_diff2_mul(llvm_state &s, const binary_operator &bo, llvm::Value *diff_arr, std::uint32_t n_uvars,
                                 llvm::Value *order, std::uint32_t idx, std::uint32_t batch_size, bool high_accuracy)
{
    return std::visit(
        [&](const auto &v1, const auto &v2) {
            return bo_taylor_diff2_mul_impl<T>(s, v1, v2, diff_arr, n_uvars, order, idx, batch_size, high_accuracy);
        },
        bo.lhs().value(), bo.rhs().value());
}
...
// Derivative of number / number.
template <typename T>
llvm::Value *bo_taylor_diff2_div_impl(llvm_state &s, const number &, const number &, llvm::Value *, std::uint32_t,
                                      llvm::Value *, std::uint32_t, std::uint32_t batch_size, bool)
{
    return create_constant_vector(s.builder(), codegen<T>(s, number{0.}), batch_size);
}
...
template <typename T, typename U,
          std::enable_if_t<std::disjunction_v<std::is_same<U, number>, std::is_same<U, variable>>, int> = 0>
llvm::Value *bo_taylor_diff2_div_impl(llvm_state &s, const U &nv, const variable &var1, llvm::Value *diff_arr,
                                      std::uint32_t n_uvars, llvm::Value *order, std::uint32_t idx,
                                      std::uint32_t batch_size, bool)
{
    auto &module = s.module();
    ...
    if (f == nullptr) {
        ...
        // Run the loop.
        // TODO overflow check.
        llvm_loop_u32(s, f, builder.getInt32(1), builder.CreateAdd(ord, builder.getInt32(1)), [&](llvm::Value *j) {
            auto cj = taylor_load_diff(s, diff_ptr, n_uvars, j, idx0);
            auto a_nj = taylor_load_diff(s, diff_ptr, n_uvars, builder.CreateSub(ord, j), idx1);
    ...
    if constexpr (std::is_same_v<U, number>) {
        ...
        ret = builder.CreateFSub(
            taylor_load_diff(s, diff_arr, n_uvars, order, builder.getInt32(uname_to_index(nv.name()))), ret);
    }

    // Compute and return the result.
    return builder.CreateFDiv(ret, taylor_load_diff(s, diff_arr, n_uvars, builder.getInt32(0),
                                                    builder.getInt32(uname_to_index(var1.name()))));
}

// All the other cases.
template <typename, typename V1, typename V2>
llvm::Value *bo_taylor_diff2_div_impl(llvm_state &, const V1 &, const V2 &, llvm::Value *, std::uint32_t, llvm::Value *,
                                      std::uint32_t, std::uint32_t, bool)
{
    assert(false);
...
template <typename T>
llvm::Value *bo_taylor_diff2_div(llvm_state &s, const binary_operator &bo, llvm::Value *diff_arr, std::uint32_t n_uvars,
                                 llvm::Value *order, std::uint32_t idx, std::uint32_t batch_size, bool high_accuracy)
{
    return std::visit(
        [&](const auto &v1, const auto &v2) {
            return bo_taylor_diff2_div_impl<T>(s, v1, v2, diff_arr, n_uvars, order, idx, batch_size, high_accuracy);
        },
        bo.lhs().value(), bo.rhs().value());
}

template <typename T>
llvm::Value *taylor_diff2_bo_impl(llvm_state &s, const binary_operator &bo, llvm::Value *diff_arr,
                                  std::uint32_t n_uvars, llvm::Value *order, std::uint32_t idx,
                                  std::uint32_t batch_size, bool high_accuracy)
{
    // lhs and rhs must be u vars or numbers.
    ...
    switch (bo.op()) {
        case binary_operator::type::add:
            return bo_taylor_diff2_add<T>(s, bo, diff_arr, n_uvars, order, idx, batch_size, high_accuracy);
        case binary_operator::type::sub:
            return bo_taylor_diff2_sub<T>(s, bo, diff_arr, n_uvars, order, idx, batch_size, high_accuracy);
        case binary_operator::type::mul:
            return bo_taylor_diff2_mul<T>(s, bo, diff_arr, n_uvars, order, idx, batch_size, high_accuracy);
        default:
            return bo_taylor_diff2_div<T>(s, bo, diff_arr, n_uvars, order, idx, batch_size, high_accuracy);
    }
}
...
llvm::Value *taylor_diff2_dbl(llvm_state &s, const binary_operator &bo, llvm::Value *diff_arr, std::uint32_t n_uvars,
                              llvm::Value *order, std::uint32_t idx, std::uint32_t batch_size, bool high_accuracy)
{
    return detail::taylor_diff2_bo_impl<double>(s, bo, diff_arr, n_uvars, order, idx, batch_size, high_accuracy);
}

llvm::Value *taylor_diff2_ldbl(llvm_state &s, const binary_operator &bo, llvm::Value *diff_arr, std::uint32_t n_uvars,
                               llvm::Value *order, std::uint32_t idx, std::uint32_t batch_size, bool high_accuracy)
{
    return detail::taylor_diff2_bo_impl<long double>(s, bo, diff_arr, n_uvars, order, idx, batch_size, high_accuracy);
}

#if defined(HEYOKA_HAVE_REAL128)

llvm::Value *taylor_diff2_f128(llvm_state &s, const binary_operator &bo, llvm::Value *diff_arr, std::uint32_t n_uvars,
                               llvm::Value *order, std::uint32_t idx, std::uint32_t batch_size, bool high_accuracy)
{
    return detail::taylor_diff2_bo_impl<mppp::real128>(s, bo, diff_arr, n_uvars, order, idx, batch_size, high_accuracy);
}
