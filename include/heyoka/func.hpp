    ...
    struct impl : public Base {
        ...
        void to_stream(std::ostringstream &oss) const final
        {
            if constexpr (requires(const T &x) { static_cast<void>(x.to_stream(oss)); }) {
                static_cast<void>(getval(this).to_stream(oss));
            } else {
                func_default_to_stream(oss, static_cast<const func_base &>(getval(this)));
            }
        }
        ...
        [[nodiscard]] bool is_time_dependent() const final
        {
            if constexpr (requires(const T &x) { static_cast<bool>(x.is_time_dependent()); }) {
                return static_cast<bool>(getval(this).is_time_dependent());
            } else {
                return false;
            }
        }
        ...
        taylor_dc_t::size_type taylor_decompose(taylor_dc_t &dc) && final
        {
            if constexpr (func_has_taylor_decompose<T>) {
                return std::move(getval(this)).taylor_decompose(dc);
            }
                                 ...
                                 std::uint32_t batch_size, bool high_accuracy) const final
        {
            if constexpr (requires(const T &x) {
                ...
                return getval(this).taylor_diff(s, fp_t, deps, arr, par_ptr, time_ptr, n_uvars, order, idx, batch_size,
                                                high_accuracy);
            } else {
                throw not_implemented_error(
                                           ...
                                           std::uint32_t batch_size, bool high_accuracy) const final
        {
            if constexpr (requires(const T &x) {
                ...
                return getval(this).taylor_c_diff_func(s, fp_t, n_uvars, batch_size, high_accuracy);
            } else {
                throw not_implemented_error(
        ...
        taylor_c_diff_get_n_iters(std::uint32_t order) const final
        {
            if constexpr (requires(const T &x) {
                ...
                return getval(this).taylor_c_diff_get_n_iters(order);
            } else {
                throw not_implemented_error(
                                                                         ...
                                                                         bool high_accuracy) const final
        {
            if constexpr (requires(const T &x) {
                ...
                return getval(this).taylor_c_diff_get_single_iter_func(s, fp_t, n_uvars, batch_size, high_accuracy);
            } else {
