...
{
    if constexpr (std::is_same_v<T, double>) {
        return taylor_init_batch_dbl(s, f, arr, batch_idx, batch_size);
    } else if constexpr (std::is_same_v<T, long double>) {
        return taylor_init_batch_ldbl(s, f, arr, batch_idx, batch_size);
#if defined(HEYOKA_HAVE_REAL128)
    } else if constexpr (std::is_same_v<T, mppp::real128>) {
        return taylor_init_batch_f128(s, f, arr, batch_idx, batch_size);
#endif
    } else {
        static_assert(detail::always_false_v<T>, "Unhandled type.");
    }
...
{
    if constexpr (std::is_same_v<T, double>) {
        return taylor_diff_batch_dbl(s, f, idx, order, n_uvars, diff_arr, batch_idx, batch_size, cd_uvars);
    } else if constexpr (std::is_same_v<T, long double>) {
        return taylor_diff_batch_ldbl(s, f, idx, order, n_uvars, diff_arr, batch_idx, batch_size, cd_uvars);
#if defined(HEYOKA_HAVE_REAL128)
    } else if constexpr (std::is_same_v<T, mppp::real128>) {
        return taylor_diff_batch_f128(s, f, idx, order, n_uvars, diff_arr, batch_idx, batch_size, cd_uvars);
#endif
    } else {
        static_assert(detail::always_false_v<T>, "Unhandled type.");
    }
